...
function buildDonut(items, total) {
  ...
  // limpiar leyenda (el gráfico ECharts se limpia con dispose)
  legend.innerHTML = "";
  ...
  // colores: mantener consistencia, forzar Stock nulo en rojo
  const palette = [
    "#1d4ed8", "#16a34a", "#f59e0b", "#7c3aed", "#0ea5e9",
    "#10b981", "#fb7185", "#a3a3a3", "#eab308", "#14b8a6"
  ];

  const norm = (s) => normalizeHeaderName(s); // ya existe helper arriba
  const isStockNulo = (name) => norm(name) === norm("Stock nulo") || norm(name) === norm("stock nulo");

  // map estado -> color
  const colorByName = {};
  let palIdx = 0;
  for (const it of items) {
    if (isStockNulo(it.estado)) {
      colorByName[it.estado] = "#ef4444"; // rojo
    } else {
    ...
    series: [{
      ...
      label: {
        ...
        formatter: (p) => {
          ...
          if (!total) return "";
          const pct = v / total;
          // mostrar solo si no es muy chico
          if (pct < 0.06) return "";
  ...
  // leyenda tipo "callouts" (igual que antes)
  items.forEach((it) => {
    const p = total ? it.qty / total : 0;
