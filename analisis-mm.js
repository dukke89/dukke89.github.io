...
function isMenorPP(name){
  return normText(name) === normText("02-MENOR A PUNTO DE PEDIDO");
}
function isMayorStockMax(name){
  return normText(name) === normText("04-MAYOR A STOCK MAXIMO") || normText(name) === normText("04-MAYOR A STOCK MAXIMO ");
}

function toDelimitedCSV(rows, delimiter=";"){
  // rows: array of objects with keys = headers
  const esc = (v) => {
    const s = (v ?? "").toString();
    // escape quotes
    const q = s.replace(/"/g, '""');
...
function downloadByEstado(kind){
  const rows = filteredRows();
  const filtered = rows.filter(r => {
    const est = clean(r[COL_ESTADO]);
    if (kind === "stock_nulo") return isStockNulo(est);
    if (kind === "menor_pp") return isMenorPP(est);
    if (kind === "mayor_stock_max") return isMayorStockMax(est);
    return false;
  });

  const cliente = getSelectedCliente() || "Todos";
  const fname = `ANALISIS-MM_${cliente}_${kind}.csv`.replace(/\s+/g, "_");
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      document.getElementById("btnDLStockNulo")?.addEventListener("click", () => downloadByEstado("stock_nulo"));
      document.getElementById("btnDLMenorPP")?.addEventListener("click", () => downloadByEstado("menor_pp"));
      document.getElementById("btnDLMayorStockMax")?.addEventListener("click", () => downloadByEstado("mayor_stock_max"));
