...
// --- FIX: eliminar botones de descarga (si quedaron de versiones anteriores) ---
function removeDownloadButtons(){
  ...
  // borrar contenedores típicos
  document.querySelectorAll(".mm-downloads,.download-buttons,.downloads,.btn-downloads").forEach(el=>el.remove());
  // borrar botones sueltos dentro del panel de filtros
  const filterPanel = document.querySelector(".mm-filter-row") || document.querySelector(".panel .panel-body");
  if(!filterPanel) return;
  filterPanel.querySelectorAll("button, a.btn, .btn").forEach(el=>{
    const t = (el.textContent||"").trim();
    if(patterns.some(p=>p.test(t)) || el.hasAttribute("data-download") || /download/i.test(el.className)){
      el.remove();
    }
  });
}
