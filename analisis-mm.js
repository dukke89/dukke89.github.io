...
const VALOR_LU_CANDIDATES = [
  "Valor libre utilización","Valor libre utilizacion","VALOR LIBRE UTILIZACION",
  "Valor Libre Utilización","Valor Libre Utilizacion","Valor libre utilización "
];
...
${pct.toFixed(0)}%`;
  ...
  // leyenda tipo "callouts" (orden 01 -> 04)
  const getPrefixNum = (name) => {
    const m = String(name || "").trim().match(/^\s*(\d{1,3})\s*[-.:_\s]/);
    return m ? parseInt(m[1], 10) : null;
  };
  const legendItems = [...items].sort((a,b)=>{
    const na = getPrefixNum(a.estado);
    const nb = getPrefixNum(b.estado);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      COL_VALOR_LU = byFirstExisting(VALOR_LU_CANDIDATES);
...
function buildValorizacionStock(rows){
  ...
  const colRubro = COL_RUBRO || "Rubro";
  const colValor = COL_VALOR_LU || "Valor libre utilización";

  // si no se encuentran las columnas, no romper el dashboard
  if (!COL_RUBRO || !COL_VALOR_LU) {
    console.warn("Valorización: no pude detectar columnas Rubro/Valor libre utilización. Headers:", headers);
  }
