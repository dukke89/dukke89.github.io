...
function getSelectedClientes() {
  // Devuelve array de clientes seleccionados (vacío = Todos)
...
function initClienteMultiSelect(){
  const root = document.getElementById("clienteMS");
  const btn = document.getElementById("clienteMSBtn");
  const panel = document.getElementById("clienteMSPanel");
  const box = document.getElementById("clienteMSOptions");
  if (!root || !btn || !panel || !box) return;
  ...
  // cualquier cambio en opciones => aplicar
...
function downloadStaticCsv(fileName){
  // Descarga directa del repo (GitHub Pages) - respeta rutas/espacios
...
function filteredRows() {
  ...
  if (!selected.length) return data; // Todos
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      renderClientes();
      initClienteMultiSelect();
      applyAll();

      // Botón descarga archivo fijo
      document.getElementById("btnDLStoNuloPP")?.addEventListener("click", () => {
        downloadStaticCsv("STO NULO - STO MENOR PP.csv");
      });
if (sel) sel.value = "";
        applyAll();
      });
