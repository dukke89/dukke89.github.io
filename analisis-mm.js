...
/* ============================
   DETALLE POR ESTADO (TABLA)
============================ */
function getSelectedEstados(){
  const chks = document.querySelectorAll(".estadoChk");
  const sel = [];
  chks.forEach(c => { if (c.checked) sel.push(c.value); });
  return sel;
}

function renderEstadoDetailTable(rows){
  const thead = document.getElementById("estadoRowsThead");
  const tbody = document.getElementById("estadoRowsTbody");
  const countEl = document.getElementById("estadoRowsCount");
  const hint = document.getElementById("estadoHint");
  if(!thead || !tbody || !countEl) return;

  const selected = getSelectedEstados();
  if(hint) hint.textContent = selected.length ? "Detalle filtrado por estado (y por cliente si aplica)." : "Seleccioná uno o más estados para ver el detalle.";

  // filtrar por estados seleccionados (match por prefijo de texto exacto; tolera espacios)
  const selectedNorm = new Set(selected.map(s => normalizeHeaderName(s)));
  const f = selected.length
    ? rows.filter(r => selectedNorm.has(normalizeHeaderName(clean(r[COL_ESTADO]))))
    : [];

  // headers del CSV: mostrar TODAS las columnas
  thead.innerHTML = "";
  headers.forEach(h => {
  ...
  tbody.innerHTML = "";
  countEl.textContent = selected.length ? `${fmtInt(f.length)} filas` : "—";

  if(!selected.length){
    tbody.innerHTML = `<tr><td class="muted">Sin filtro seleccionado</td></tr>`;
    return;
  }
  if(!f.length){
    tbody.innerHTML = `<tr><td class="muted">Sin filas para el/los estado(s) seleccionado(s)</td></tr>`;
    return;
  }

  // limitar render para no colgar el navegador si hay miles (igual el usuario puede filtrar por cliente)
  const MAX = 800;
  const shown = f.slice(0, MAX);

  for(const r of shown){
    const tr = document.createElement("tr");
    headers.forEach(h => {
      const td = document.createElement("td");
      td.textContent = clean(r[h]);
      tr.appendChild(td);
    });
  ...
  if(f.length > MAX){
    ...
    td.className = "muted";
    td.textContent = `Mostrando ${MAX} de ${fmtInt(f.length)} filas. Usá el filtro de CLIENTE para acotar.`;
    tr.appendChild(td);
    tbody.appendChild(tr);
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      // filtros de estados (detalle)
      document.querySelectorAll(".estadoChk").forEach(chk => chk.addEventListener("change", applyAll));
      document.getElementById("btnEstadosReset")?.addEventListener("click", () => {
        document.querySelectorAll(".estadoChk").forEach(chk => chk.checked = false);
        applyAll();
      });
})
    .catch(err => {
