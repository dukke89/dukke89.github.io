...
/* ============================
   CSV LOADER (robusto para GitHub Pages)
============================ */
function guessDelimiter(text) {
  ...
  for (const d of candidates) {
    const score = head.split(d).length;
...
function buildCsvCandidates(fileName) {
  ...
  const path = window.location.pathname.replace(/\/[^\/]*$/, "/"); // dir actual
  const repo = window.location.pathname.split("/").filter(Boolean)[0] || "";
  const names = Array.from(new Set([fileName, fileName.toLowerCase(), fileName.toUpperCase()]));
  const urls = [];
  for (const n of names) {
    urls.push(origin + path + n);
    urls.push(origin + "/" + n);
    if (repo) urls.push(origin + "/" + repo + "/" + n);
    urls.push("./" + n);
  }
...
async function fetchTextWithFallback(fileName) {
  const tries = buildCsvCandidates(fileName);
  const errors = [];
  for (const url of tries) {
    try {
      ...
      const sample = (t || "").slice(0, 200).toLowerCase();
      if (sample.includes("<!doctype") || sample.includes("<html")) throw new Error("Recibí HTML (no CSV)");
      return { url, text: t, tries };
    } catch (e) {
      errors.push(url + " → " + (e?.message || e));
    }
  }
  const err = new Error("No pude cargar el CSV");
  err.details = errors;
  err.tries = tries;
  throw err;
}
...
/* ============================
   CONFIG
============================ */
const CSV_FILE = "ANALISIS-MM.csv";

const DELIM = ";";
...
function showError(msg) {
  ...
  if (el) el.innerHTML = `<div class="error">${msg}
...
function showLoading(msg="Cargando datos…"){
  const el = document.getElementById("msg");
  if (el) el.innerHTML = `<div class="notice loading"><span class="spinner"></span><span>${msg}</span></div>`;
}
...
${pct.toFixed(0)}%`;
  ...
  if (!donutResizeBound){
    ...
    window.addEventListener("resize", () => { try { chartDonut && chartDonut.resize(); } catch(e) {} }, { passive: true });
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetchTextWithFallback(CSV_FILE)
    .then(({url, text, tries}) => {
      // Debug visible en UI
      try { showInfo(`CSV: ${url}`); } catch(e) {}
    ...
    .catch(err => {
      ...
      if (err && err.details && Array.isArray(err.details)) {
        extra = "<br><div class=\"muted\">Probé:<br>" + err.details.slice(0,10).map(x=>"- "+x).join("<br>") + (err.details.length>10? "<br>…": "") + "</div>";
      }
      try { console.error("CSV load error", err); } catch(e) {}
      showError(msg + extra);
    })
});
