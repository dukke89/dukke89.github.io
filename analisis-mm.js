...
${pct.toFixed(0)}%`;
  ...
  // leyenda tipo "callouts" + botones a la derecha (como referencia)
  const prefixNum = (name) => {
    const m = String(name || "").trim().match(/^\s*(\d{1,2})\s*[-.:_\s]/);
    return m ? Number(m[1]) : null;
  };

  const labelByPrefix = (n) => {
  ...
  orderedItems.forEach((it) => {
    ...
    // Botón descarga (solo para prefijos 01-04)
    const pnum = prefixNum(it.estado);
    const lbl = labelByPrefix(pnum);
    if (lbl){
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "mm-legend-btn";
      btn.textContent = `\u2B07 ${lbl}`;
      btn.title = `Descargar ${lbl} según filtro seleccionado`;
      btn.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        downloadByEstadoName(it.estado, lbl.toUpperCase().replace(/\s+/g, "_"));
      });
...
/* ============================
   DOWNLOAD: filtered CSV by Estado (leyenda)
============================ */
...
function rowsToCsv(rows){
  // rows: array of objects keyed by headers
  const lines = [];
  lines.push(headers.map(csvEscape).join(DELIM));
  for (const r of rows){
    const line = headers.map(h => csvEscape(r[h])).join(DELIM);
    lines.push(line);
  }
  return lines.join("\n");
}

function triggerDownloadBlob(filename, text, mime="text/csv;charset=utf-8"){
...
function buildFileSuffixFromFiltro(){
  const sel = getSelectedClientes();
  if (!sel || !sel.length) return "TODOS";
  // sanitiza para nombre de archivo
  const joined = sel.join("_").slice(0, 80);
  return joined.replace(/[^a-zA-Z0-9_\-]/g, "_");
}

function downloadByEstadoName(estadoName, labelForFile){
  const rows = filteredRows().filter(r => clean(r[COL_ESTADO]) === clean(estadoName));
  const suffix = buildFileSuffixFromFiltro();
  const safeLabel = (labelForFile || estadoName || "ESTADO").toString().trim().replace(/[^a-zA-Z0-9_\-]/g, "_");
  const filename = `${safeLabel}_${suffix}.csv`;
  const csv = rowsToCsv(rows);
  triggerDownloadBlob(filename, csv);
}
