...
/* ============================
   CSV FETCH (fallback robusto)
   - Soporta GitHub Pages en subcarpetas
============================ */
function basePrefix(){
  // Para project pages: /repo/... => "/repo/"
  const parts = (location.pathname || "/").split("/").filter(Boolean);
  if (parts.length >= 1) return "/" + parts[0] + "/";
  return "/";
}
function buildCsvUrls(file){
  const urls = [];
  // mismo directorio (respeta subcarpetas)
  urls.push(new URL(file, location.href).toString());
  // relativo explícito
  urls.push("./" + file);
  // raíz del host
  urls.push("/" + file);
  // raíz del repo (project pages)
  urls.push(basePrefix() + file);
  return [...new Set(urls)];
}
async function fetchTextWithFallback(file){
  const urls = buildCsvUrls(file);
  let lastErr = null;
  ...
  throw new Error(`No pude abrir el CSV "${file}". Probé: ${urls.join(" | ")}. Último error: ${lastErr?.message || lastErr}`);
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetchTextWithFallback(CSV_FILE)
    .then(r => {
      if (!r.ok) throw new Error(`No pude abrir ${CSV_FILE} (HTTP ${r.status})`);
      return r.text();
    })
    .then(text => {
