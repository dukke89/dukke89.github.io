...
function buildDonut(items, total) {
  // Reemplazo Plotly/D3 por Apache ECharts manteniendo la misma estructura (donutEstados + donutLegend)
  if (!window.echarts) {
  ...
  // limpiar host y leyenda (como antes)
  host.innerHTML = "";
  legend.innerHTML = "";

  // crear un div interno para ECharts (así no dependemos de SVG)
  const chartDiv = document.createElement("div");
  chartDiv.style.width = "100%";
  chartDiv.style.height = "440px";
  host.appendChild(chartDiv);

  // Paleta tipo Tableau10 (similar a D3 schemeTableau10)
  const palette = ["#4e79a7","#f28e2b","#e15759","#76b7b2","#59a14f","#edc948","#b07aa1","#ff9da7","#9c755f","#bab0ab"];

  // Color estable por estado + override STOCK NULO -> ROJO
  const colorMap = new Map();
  let pi = 0;

  const norm = (s) => normalizeHeaderName(String(s || ""));
  const isStockNulo = (estado) => norm(estado).includes("stock nulo");

  function getColor(estado){
    if (isStockNulo(estado)) return "#d32f2f"; // 🔴 pedido: STOCK NULO en rojo
    if (!colorMap.has(estado)){
      colorMap.set(estado, palette[pi % palette.length]);
      pi++;
    }
    return colorMap.get(estado);
  }

  // Armar data y detectar mayor para "separar" (selectedOffset)
  let maxIdx = 0;
  items.forEach((it, i) => { if (it.qty > (items[maxIdx]?.qty || 0)) maxIdx = i; });

  const seriesData = items.map((it, i) => ({
    name: it.estado,
    value: it.qty,
    itemStyle: { color: getColor(it.estado) },
    selected: i === maxIdx
  }));
  ...
  chart.setOption({
    tooltip: {
      ...
      formatter: (p) => {
        ...
        return `${p.name}<br/>${fmtInt(p.value)} materiales (${pct.toFixed(0)}%)`;
      }
    },
    series: [{
      name: "Estados",
  ...
  // Callouts en leyenda (igual a tu referencia)
  items.forEach((it) => {
    const p = total ? it.qty / total : 0;
    const pct = (p * 100).toFixed(0) + "%";
    const c = getColor(it.estado);
