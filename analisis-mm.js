...
function buildDonut(items, total) {
  // Apache ECharts (pie) + callouts a la derecha
  if (!window.echarts) {
  ...
  // colores consistentes (Tableau-like)
  const palette = [
  ...
  // porción mayor "separada"
  let maxIdx = 0;
  data.forEach((d, i) => { if (d.value > (data[maxIdx]?.value ?? -Infinity)) maxIdx = i; });
  if (data[maxIdx]) data[maxIdx].selected = true;
  ...
  // Callouts en leyenda (como tu diseño actual)
  data.forEach((d) => {
