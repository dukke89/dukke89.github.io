...
${pct.toFixed(0)}%`;
  ...
  orderedItems.forEach((it) => {
    ...
    // Botón descarga (solo para los 4 estados principales)
    const kind = kindFromEstadoName(it.estado);
    if (kind){
      ...
      btn.type = "button";
      btn.className = "btn btn-outline mm-dl-btn";
      btn.setAttribute("data-kind", kind);
      btn.title = `Descargar: ${labelForKind(kind)}`;
      btn.textContent = `⬇ ${labelForKind(kind)}`;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (ev) => {
    ...
    if (!t) return;
    // Botones de descarga por estado (en la leyenda)
    if (t.classList && t.classList.contains("mm-dl-btn")) {
      const kind = t.getAttribute("data-kind");
      if (kind) return downloadByKind(kind);
    }
...
/* ============================
   DOWNLOADS (ANALISIS-MM filtrado por CLIENTE/ALMACEN)
============================ */

/* Escapa valores para CSV con ; */
...
/* Normaliza texto para comparar estados */
function normLooseText(s){
  return normalizeHeaderName(s)
    .replace(/^[0-9]+\s*[-.:_\s]*/g, "")   // quita prefijo "01-" etc.
    .replace(/[_\-\s]+/g, " ")
    .trim();
}

/* Identifica qué tipo de descarga corresponde a un estado (01..04) */
function kindFromEstadoName(estado){
  const t = normLooseText(estado);

  // 01
  if (t === "stock nulo" || t.includes("stock nulo")) return "stock_nulo";

  // 02
  if (
    t.includes("menor a punto de pedido") ||
    t.includes("menor al punto de pedido") ||
    t.includes("menor a pp") ||
    t.includes("menor al pp")
  ) return "menor_pp";

  // 03
  if (
    t.includes("mayor a punto de pedido") ||
    t.includes("mayor al punto de pedido") ||
    t.includes("mayor a pp") ||
    t.includes("mayor al pp")
  ) return "mayor_pp";

  // 04
  if (
    t.includes("mayor a stock maximo") ||
    t.includes("mayor al stock maximo") ||
    t.includes("mayor stock maximo") ||
    t.includes("mayor a stock maximo")
  ) return "mayor_stock_max";
...
function labelForKind(kind){
  switch(kind){
    case "stock_nulo": return "Stock nulo";
    case "menor_pp": return "Menor al PP";
    case "mayor_pp": return "Mayor al PP";
    case "mayor_stock_max": return "Mayor Stock Max";
    default: return "Descargar";
  }
}

function filterRowsByKind(rows, kind){
  return rows.filter(r => {
    const est = clean(r[COL_ESTADO]);
    return kindFromEstadoName(est) === kind;
  });
}
...
function downloadByKind(kind){
  const rows = filteredRows();
  const out = filterRowsByKind(rows, kind);

  const clientes = getSelectedClientes();
  const scope = clientes.length ? clientes.join("_") : "Todos";
  const filename = `ANALISIS-MM_${slug(labelForKind(kind))}_${slug(scope)}.csv`;

  if (!out.length){
    showError(`No hay filas para descargar en "${labelForKind(kind)}" con el filtro actual.`);
    return;
  }

  downloadBlob(filename, rowsToCsv(out));
}
