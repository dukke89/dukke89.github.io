  ...
  orderedItems.forEach((it) => {
    ...
    if (btnLabel && fileName) {
      ...
      btn.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        // ✅ Stock nulo (01) descarga "STO NULO.csv" (y el resto según el mapa)
        downloadByEstadoValue(btnLabel, fileName);
      });
...
// === PATCH: DESCARGA POR ESTADO (FILTRADA DESDE ANALISIS-MM.csv) ===
function downloadByEstadoValue(estadoLabel, filename){
  if (!headers || !headers.length || !data || !data.length) {
    showError("Todavía no se cargó el archivo ANALISIS-MM.csv.");
    return;
  }
  const rows = filteredRows();
  const wanted = normalizeHeaderName(estadoLabel);
  const out = rows.filter(r => normalizeHeaderName(r[COL_ESTADO]) === wanted);

  if (!out.length){
    showError(`No hay filas para descargar para el estado: ${estadoLabel}`);
    return;
  }
  const csvText = toDelimitedCSV(headers, out, DELIM);
