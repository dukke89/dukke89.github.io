...
function getSelectedClientes() {
  ...
  // Fallback: select simple (compatibilidad)
...
function renderClientes() {
  ...
  // llenar multiselect si existe
  const root = document.getElementById("clienteMS");
  const btn = document.getElementById("clienteMSBtn");
  const panel = document.getElementById("clienteMSPanel");
  const box = document.getElementById("clienteMSOptions");
  const allCb = panel ? panel.querySelector("input[data-all='1']") : null;

  if (root && btn && panel && box && allCb) {
    box.innerHTML = "";
    clientes.forEach(c => {
    ...
    // abrir/cerrar
    btn.onclick = () => root.classList.toggle("open");
...
// descarga directa del archivo del repo
document.getElementById("btnDLStoNuloPP")?.addEventListener("click", () => {
  const file = "STO NULO - STO MENOR PP.csv";
