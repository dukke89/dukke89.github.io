  ...
  orderedItems.forEach((it) => {
    ...
    if (btnLabel) {
      ...
      btn.addEventListener("click", (ev) => {
        ...
        // ✅ ACÁ definís el nombre del archivo que querés para STOCK NULO
        //    (cambiá solo el string si querés otro nombre)
        const forcedFilename = (prefNum === 1) ? "STO NULO.csv" : null;

        downloadByEstadoName(it.estado, forcedFilename, btnLabel);
      });
...
/* ============================
   DOWNLOAD: Leyenda (filtrado por Cliente/Almacén + Estado)
============================ */
...
function downloadByEstadoName(estadoName, forcedFilename=null, labelForFallback="ESTADO"){
  const rows = filteredRows().filter(r => clean(r[COL_ESTADO]) === clean(estadoName));
  const safeLabel = (labelForFallback || estadoName || "ESTADO").toString().trim().replace(/[^a-zA-Z0-9_\-]/g, "_");
  const filename = forcedFilename || (safeLabel + ".csv");
