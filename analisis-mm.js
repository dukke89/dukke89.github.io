...
/* ============================
   DOWNLOAD (filtrado por Cliente/Almacén + Estado)
============================ */
function toCsvText(rows, hdrs, delim = DELIM){
  const esc = (v) => {
    const s = (v ?? "").toString();
  ...
  const lines = [];
  lines.push(hdrs.map(esc).join(delim));
  rows.forEach(r => {
    lines.push(hdrs.map(h => esc(r[h])).join(delim));
  });
  return lines.join("\r\n");
}
function downloadTextFile(filename, text){
  const bom = "\ufeff"; // para Excel (acentos)
...
function downloadByEstado(estadoName){
  const rows = filteredRows();
  const normEq = (a,b) => normLoose(a) === normLoose(b);
  const out = rows.filter(r => normEq(r[COL_ESTADO], estadoName));
  if (!out.length){
    alert("No hay filas para descargar con el filtro actual.");
    return;
  }
  const safeName = String(estadoName || "estado").replace(/[\\/:*?"<>|]+/g, " ").trim();
  const filename = `ANALISIS-MM_${safeName}.csv`;
  downloadTextFile(filename, toCsvText(out, headers, DELIM));
}
function estadoButtonLabel(estadoName){
  const n = normLoose(estadoName);
  if (n.includes("stock nulo")) return "↓ Stock nulo";
  if (n.includes("menor") && (n.includes("punto de pedido") || n.includes("pp"))) return "↓ Menor al PP";
  if (n.includes("mayor") && (n.includes("punto de pedido") || n.includes("pp"))) return "↓ Mayor al PP";
  if (n.includes("mayor") && (n.includes("stock maximo") || n.includes("stock máximo") || n.includes("stock max"))) return "↓ Mayor Stock Max";
  return "↓ Descargar";
}


function downloadByKind(kind){
  // compat: descarga por "kind" si algún botón viejo lo usa
  const kindToEstado = {
    "stock_nulo": "01-STOCK NULO",
    "menor_pp": "02-MENOR A PUNTO DE PEDIDO",
    "mayor_pp": "03-MAYOR A PUNTO DE PEDIDO",
    "mayor_stock_max": "04-MAYOR A STOCK MAXIMO"
  };
  const est = kindToEstado[kind] || kind;
  return downloadByEstado(est);
}
...
${pct.toFixed(0)}%`;
  ...
  orderedItems.forEach((it) => {
    ...
    // Botón de descarga (filtra por Cliente/Almacén + este Estado)
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "btn-descarga-estado";
    btn.dataset.estado = it.estado;
    btn.textContent = estadoButtonLabel(it.estado);
...
// Delegación de eventos: asegura que los botones funcionen siempre
  document.addEventListener("click", (ev) => {
    ...
    if (!t) return;
    if (t.classList && t.classList.contains('btn-descarga-estado')) return downloadByEstado(t.dataset.estado);
    
    if (t.id === "btnDLStockNulo") return downloadByKind("stock_nulo");
    if (t.id === "btnDLMenorPP") return downloadByKind("menor_pp");
    if (t.id === "btnDLMayorPP") return downloadByKind("mayor_pp");
