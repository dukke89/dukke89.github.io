...
function animateValue(id, start, end, duration, isPct = false) {
  ...
  if (!obj) return;
  if (start === end) {
    obj.textContent = isPct ? fmtPct(end) : fmtInt(end);
    return;
  }
  ...
  function step(timestamp) {
    ...
    obj.textContent = isPct ? fmtPct(current) : fmtInt(Math.floor(current));
    if (progress < 1) {
...
function renderClientes() {
  ...
  for (const c of clientes) {
    ...
    cb.type = "checkbox";
    cb.value = c;
    cb.checked = (prevChecked.size === 0 && allWasChecked) || prevChecked.has(c);
    lbl.appendChild(cb);
  ...
  chartDonut.setOption({
    tooltip: {
      ...
      formatter: (p) => {
        ...
        const val = fmtInt(p.value);
        const col = p.color || '#64748b';
        return `
          <div style="padding: 10px 14px; border-left: 5px solid ${col};">
            <div style="font-size: 11px; color: #64748b; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">Disponibilidad</div>
            <div style="font-size: 14px; color: #1e293b; font-weight: 800; margin-bottom: 4px;">${p.name}</div>
            <div style="display: flex; align-items: baseline; gap: 8px;">
              <span style="font-size: 18px; font-weight: 900; color: #0f172a;">${val}</span>
...
function applyAll() {
  ...
  // KPI Materiales
  const prevMat = parseInt(document.getElementById("kpiMat").dataset.v || "0");
  document.getElementById("kpiMat").dataset.v = k.totalMat;
  animateValue("kpiMat", prevMat, k.totalMat, 800);

  // KPI Disponible
  const prevDisp = parseInt(document.getElementById("kpiDisp").dataset.v || "0");
  document.getElementById("kpiDisp").dataset.v = k.dispMat;
  animateValue("kpiDisp", prevDisp, k.dispMat, 800);

  // KPI Pct
  const prevPct = parseFloat(document.getElementById("kpiPct").dataset.v || "0");
  document.getElementById("kpiPct").dataset.v = k.pct;
  animateValue("kpiPct", prevPct, k.pct, 800, true);
