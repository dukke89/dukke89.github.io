...
function downloadExternalFilteredCSV(filename) {
  ...
  fetch(filename)
    ...
    .then(text => {
      ...
      // We must match the Almacen column (values like 0223), not Cliente (values like 00223 Comp Amb...)
      // Let's first try to find the exact COL_CLIENT used in ANALISIS-MM
      let colIdx = fileHeaders.indexOf(COL_CLIENT);
      
      // If exact match fails, find any column that normalizes to 'almacen'
      if (colIdx === -1) {
        colIdx = fileHeaders.findIndex(h => normalizeHeaderName(h) === "almacen");
      }
      
      // Fallback to any of the candidates
      if (colIdx === -1) {
      ...
      const filteredRowsText = m.filter((row, idx) => {
        ...
        return selectedClientes.includes(val);
