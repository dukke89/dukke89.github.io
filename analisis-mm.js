...
const ESTADO_DOWNLOAD_FILES = {
  ...
  2: "MENOR A PP.csv",         // 02 - Menor a Punto de Pedido (si existe)
  ...
  orderedItems.forEach((it) => {
    ...
    if (btnLabel && fileName) {
      ...
      btn.addEventListener("click", (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        // 01 Stock nulo y 02 Menor al PP: quedan estáticos. 03/04: se filtran de ANALISIS-MM.csv por columna ESTADO.
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl + "?t=" + window.CACHE_BUSTER)
    .then(r => {
    ...
    .then(jsonData => {
      ...
      if (missing.length) {
        showError(
          `Faltan columnas en el JSON: ${missing.join(", ")}<br>` +
...
function downloadExternalFilteredCSV(filename) {
  ...
  fetch(filename + "?t=" + window.CACHE_BUSTER)
    .then(r => {
      if (!r.ok) throw new Error("No se pudo descargar " + filename);
      return r.text();
    })
    .then(text => {
      ...
      // Let's explicitly search for a column containing 'almac' (to cover Almacén, Almacn, ALMACEN)
      let colIdx = fileHeaders.findIndex(h => {
        const norm = normalizeHeaderName(h);
        return norm.includes("almac") && !norm.includes("cliente"); // match 'almacen' directly
      });
      
      // If exact match fails, fallback to COL_CLIENT match
      if (colIdx === -1) {
        colIdx = fileHeaders.indexOf(COL_CLIENT);
      }
      
      // Fallback to any of the candidates if still not found
      if (colIdx === -1) {
      ...
      if (colIdx === -1) {
        // Fallback if we can't find Almacen column in the external file
        downloadStaticFile(filename);
        return;
      }
      ...
      const filteredRowsText = m.filter((row, idx) => {
        ...
        return selectedClientes.some(sc => {
          ...
          // Si ambos parecen números, los comparamos como números para ignorar ceros a la izquierda (ej. "0223" == "223")
