...
function buildValorizacionStock(rows){
  ...
  const map = new Map();
  rows.forEach(r => {
    const rub = r[colRubro];
    const v = Number(String(r[colValor] || 0).replace(/[^0-9,-]+/g,"").replace(",", ".")) || 0;
    if (!rub) return;
    map.set(rub, (map.get(rub) || 0) + v);
  });
  ...
  data.forEach(d => {
    ...
    tr.innerHTML = `
      ...
      <td>${pctAcc.toFixed(2).replace(".", ",")}%</td>
    `;
