...
function __applyEchartsPolish(option){
  option = option || {};
  option.textStyle = option.textStyle || {};
  option.textStyle.fontFamily = "Source Sans 3, system-ui, -apple-system, Segoe UI, Roboto, Arial";
  option.textStyle.color = option.textStyle.color || __cssVar("--chart-ink", "#0b1220");

  const gridColor = __cssVar("--chart-grid", "rgba(82,97,114,.18)");
  const muted = __cssVar("--chart-muted", "#526172");

  ["xAxis","yAxis"].forEach(axKey=>{
    const ax = option[axKey];
    if(!ax) return;
    const arr = Array.isArray(ax) ? ax : [ax];
    arr.forEach(a=>{
      a.axisLine = a.axisLine || {};
      a.axisLine.lineStyle = a.axisLine.lineStyle || {};
      a.axisLine.lineStyle.color = a.axisLine.lineStyle.color || gridColor;
      a.axisTick = a.axisTick || {};
      a.axisTick.lineStyle = a.axisTick.lineStyle || {};
      a.axisTick.lineStyle.color = a.axisTick.lineStyle.color || gridColor;
      a.axisLabel = a.axisLabel || {};
      a.axisLabel.color = a.axisLabel.color || muted;
      a.splitLine = a.splitLine || {};
      a.splitLine.lineStyle = a.splitLine.lineStyle || {};
      a.splitLine.lineStyle.color = a.splitLine.lineStyle.color || gridColor;
    });
  });

  if(option.legend){
    const legs = Array.isArray(option.legend) ? option.legend : [option.legend];
    legs.forEach(l=>{
      l.textStyle = l.textStyle || {};
      l.textStyle.color = l.textStyle.color || muted;
      l.textStyle.fontWeight = l.textStyle.fontWeight || 700;
    });
  }

  option.tooltip = option.tooltip || {};
  option.tooltip.backgroundColor = option.tooltip.backgroundColor || "rgba(255,255,255,.96)";
  option.tooltip.borderColor = option.tooltip.borderColor || gridColor;
  option.tooltip.textStyle = option.tooltip.textStyle || {};
  option.tooltip.textStyle.color = option.tooltip.textStyle.color || __cssVar("--chart-ink", "#0b1220");
  option.tooltip.extraCssText = (option.tooltip.extraCssText || "") + "border-radius:12px;box-shadow:0 12px 28px rgba(2,8,20,.16);";
...
function getSelectedClientes() {
  ...
  const wrap = document.getElementById("clienteMSOptions");
  if (!wrap) return [];
...
function renderClientes() {
  const box = document.getElementById("clienteMSOptions");
  const btn = document.getElementById("clienteMSBtn");
  const panel = document.getElementById("clienteMSPanel");
  const root = document.getElementById("clienteMS");
  if (!box || !btn || !panel || !root) return;
  ...
  clientes.forEach(c => {
    ...
    row.innerHTML = `<input type="checkbox" value="${c}"><span>${c}</span>`;
    box.appendChild(row);
  });
  ...
  function positionPanel(){
    ...
    panel.style.top  = `${Math.min(window.innerHeight - 8, r.bottom + 6)}px`;
    panel.style.width = `${Math.min(520, Math.max(320, r.width))}px`;
  ...
  btn.onclick = () => { root.classList.contains("open") ? closePanel() : openPanel(); };

  document.addEventListener("click", (e) => {
    if (!root.contains(e.target)) closePanel();
  });

  window.addEventListener("resize", () => { if (root.classList.contains("open")) positionPanel(); }, { passive:true });
  window.addEventListener("scroll",  () => { if (root.classList.contains("open")) positionPanel(); }, { passive:true });
