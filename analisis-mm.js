...
function buildValorizacionStock(rows){
  ...
  function isRubroAlert(v){
    ...
    if (!s) return true; // vacío
    const up = s.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    return up === "?" || up === "OBSOLETO" || up === "OTROS";
  }

  rows.forEach(r => {
    const raw = (r[colRubro] ?? "").toString();
    const rub = raw.trim();
    // incluir vacíos (los mostramos como "(Vacío)")
    const key = rub ? rub : "(Vacío)";
    const val = parseMoney(r[colValor]);
    agg.set(key, (agg.get(key) || 0) + (isFinite(val) ? val : 0));
  });
  ...
  data.forEach(d => {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td class="${isRubroAlert(d.rubro) ? 'rubro-alert' : ''}">${d.rubro}</td>
