...
window.addEventListener("DOMContentLoaded", () => {
  // Reposicionar dropdown del multiselect si hay scroll/resize
  window.addEventListener("resize", () => {
    const root = document.getElementById("clienteMS");
    if (root && root.classList.contains("open")) positionClientePanel();
  }, { passive:true });
...
function openClientePanel(){
  const root = document.getElementById("clienteMS");
  if (!root) return;
  root.classList.add("open");
  positionClientePanel();
}
