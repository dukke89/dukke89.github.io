...
// ----- ECharts helpers (reuse instance + single resize handler)
function getEChart(id){
  window.__echartsCharts = window.__echartsCharts || {};
  if(!window.__echartsResizeBound){
    window.__echartsResizeBound = true;
for(const k in m){ try{ m[k] && m[k].resize && m[k].resize(); }catch(e){} }
    }, { passive: true });
  }
  const el = document.getElementById(id);
  if(!el) return null;
  const prev = window.__echartsCharts[id];
  if(prev && !prev.isDisposed()) return prev;
  const c = echarts.init(el);
  window.__echartsCharts[id] = c;
  return c;
}
      ...
      function render(){
        ...
        const seriesLine = {
          ...
          data: months.map(m=> avgPct(m))
                ...
                const chart = getEChart("chartEvolucion");
        if(!chart) return;
