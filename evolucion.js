...
// Quote-safe CSV parser (igual al de analisis-mm.js)
...
function initEvolucion(){
  ...
  fetch(EVOL_URL)
    ...
    .then(text => {
      let m = parseDelimited(text, EVOL_DELIM);
      if(!m.length || m.length < 2) m = parseDelimited(text, ",");
      ...
      const colPct   = findColByKeywords(["dispon"]);

      const colItemsMM = findColByKeywords(["items","mm"]) || findColByKeywords(["item","mm"]);
      const colStockNulo = findColByKeywords(["stock","nulo"]);
      ...
      function render(){
        ...
        rows.forEach(d => {
          ...
          map[mon].pctVals.push(toNumber(d[colPct]));
        });
        ...
        const colorByCol = {};
        if(colStockNulo) colorByCol[colStockNulo] = "#e53935";
        if(colMenorPP) colorByCol[colMenorPP] = "#fb8c00";
        if(colMayorPP) colorByCol[colMayorPP] = "#2e7d32";
        if(colMayorStockMax) colorByCol[colMayorStockMax] = "#1b5e20";
