...
function buildChartMes() {
  // ✅ Migrado a Apache ECharts: barras por ÁREA (mes en eje X)
  ...
  // valores por área y mes
const valuesByArea = areaNames.map(a =>
  months.map(m => {
...
const monthTotals = months.map((m, idx) =>
  valuesByArea.reduce((acc, arr) => acc + (Number(arr[idx]) || 0), 0)
);
...
const seriesBars = areaNames.map((a, ai) => ({
  ...
  label: {
    ...
    formatter: (p) => {
      ...
      if (!v) return "";
      const tot = Number(monthTotals[p.dataIndex]) || 0;
      const pct = tot ? (v * 100) / tot : 0;
      return `${fmtInt(v)} - ${pct.toFixed(1)}% - ${p.seriesName}`;
    },
    fontWeight: 700
  },
  ...
  const option = {
    ...
    grid: { left: 50, right: 30, top: 70, bottom: 120, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: {
