...
// --- Helper: formatear porcentaje (xx,x%) ---
function fmtPct(v, decimals = 1) {
  const n = Number(v);
  const d = Number.isFinite(decimals) ? decimals : 1;
  if (!Number.isFinite(n)) return (0).toFixed(d).replace(".", ",") + "%";
...
function buildMesSelect(rows) {
  ...
  // Mostrar solo los últimos 4 meses (scroll visible)
  const _visibleMonths = 4;
  const _nMonths = months.length;
  const _zoomStart = _nMonths > _visibleMonths ? (100 - (_visibleMonths / _nMonths) * 100) : 0;
...
function buildChartMes() {
  ...
  // total de demoras por mes (cantidad de filas en ese mes)
  const monthTotals = months.map(m => rows.filter(r => getMonthKeyFromRow(r) === m).length);
  ...
  const seriesBars = areaNames.map(a => ({
    ...
    label: {
      ...
      formatter: (p) => {
        ...
        return `${fmtInt(v)} - ${fmtPct(pct)} - ${p.seriesName}`;
      }
    }
  }));
  ...
  const option = {
    ...
    grid: { left: 40, right: 20, top: 20, bottom: 160, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: {
    ...
    dataZoom: [
      {
        ...
        fillerColor: "rgba(20,120,90,0.18)",
        backgroundColor: "rgba(0,0,0,0.05)"
      },
      { type: "inside", start: _zoomStart, end: 100 }
    ],
    series: [
      ...seriesBars]
  };
