...
function buildChartMes() {
  ...
  const areaNames = AREA_COLS.slice(); // estable
    // total por mes (suma de todas las áreas) para calcular % en etiquetas
  const totalsByMonth = months.map(mKey => {
    const rowsM = rows.filter(r => getMonthKeyFromRow(r) === mKey);
    const mp = aggAreas(rowsM);
    let t = 0;
    for (const a of areaNames) t += (mp.get(a) || 0);
    return t;
  });
...
const seriesBars = areaNames.map(a => ({
    ...
    // Etiqueta como el ejemplo: nombre (serie/área) + valor dentro de la barra, rotado.
    label: {
      show: true,
      ...
      formatter: (p) => {
        if (!p.value) return "";
        const total = totalsByMonth[p.dataIndex] || 0;
        const pct = total ? (p.value / total) * 100 : 0;
        // Ej: CADENA DE SUMINISTRO
394 (27,8%)
        return `${p.seriesName}
${fmtInt(p.value)} (${pct.toFixed(1).replace(".", ",")}%)`;
      }
    }
  }));
  ...
  const option = {
    ...
    legend: { bottom: 60, type: "scroll" },
    grid: { left: 40, right: 20, top: 20, bottom: 95, containLabel: true },
    // Mostrar solo ~5 meses y navegar con barra (slider)
    dataZoom: [
      {
        type: "slider",
        xAxisIndex: 0,
        startValue: 0,
        endValue: Math.min(4, months.length - 1),
        height: 28,
        bottom: 24,
        showDetail: false,
        backgroundColor: "rgba(0,0,0,0.08)",
        borderColor: "rgba(0,0,0,0.25)",
        fillerColor: "rgba(0,120,0,0.28)",
        handleSize: 18,
        handleStyle: {
        ...
        dataBackground: {
          ...
          areaStyle: { color: "rgba(0,0,0,0.10)" }
        },
        textStyle: { color: "rgba(0,0,0,0.80)", fontWeight: 800 }
      },
      {
        type: "inside",
        xAxisIndex: 0,
