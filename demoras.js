...
function buildChartMes() {
  // ✅ Apache ECharts: barras por ÁREA (mes en eje X)
  // - Muestra solo los últimos 4 meses con barra de desplazamiento (dataZoom)
  // - Etiqueta por barra: "cantidad - % - área"
  ...
  // matriz [area][mes] con conteos
  const matrix = areaNames.map(a => months.map(m => {
    const rowsM = rows.filter(r => getMonthKeyFromRow(r) === m);
    const mp = aggAreas(rowsM);
    return mp.get(a) || 0;
  }));

  // total por mes (sum de áreas) para calcular porcentajes
  const monthTotals = months.map((m, idx) => {
    return matrix.reduce((acc, arr) => acc + (Number(arr[idx]) || 0), 0);
  });

  const seriesBars = areaNames.map((a, ai) => ({
    name: a,
    type: "bar",
    barMaxWidth: 34,
    data: matrix[ai],
    label: {
      ...
      formatter: (p) => {
        ...
        if (!v) return "";
        const totalMes = monthTotals[p.dataIndex] || 0;
        const pct = totalMes ? (v / totalMes * 100) : 0;
        return `${fmtInt(v)} - ${pct.toFixed(1).replace(".", ",")}% - ${p.seriesName}`;
      }
    },
  ...
  // mostrar últimos 4 meses (si hay menos, muestra todos)
  const endIdx = Math.max(0, months.length - 1);
  const startIdx = Math.max(0, months.length - 4);

  const option = {
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" } },
    legend: { top: 6, type: "scroll" },
    grid: { left: 44, right: 18, top: 46, bottom: 78, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: { rotate: 35, fontWeight: 800 }
    },
    yAxis: { type: "value" },
    dataZoom: [
      {
        type: "slider",
        show: true,
        startValue: months[startIdx],
        endValue: months[endIdx],
        bottom: 14,
        height: 22,
        borderColor: "rgba(11,93,70,0.35)",
        backgroundColor: "rgba(203,213,225,0.35)",
        fillerColor: "rgba(11,93,70,0.25)",
        handleSize: "140%",
        handleStyle: { color: "#0b5d46" },
        moveHandleSize: 0,
        textStyle: { color: "#0b1220", fontWeight: 800 }
      },
      { type: "inside" }
    ],
    series: seriesBars
