/* ============================
   DEMORAS - CONFIG
============================ */
const csvUrl = "./DEMORAS.csv";   // OJO: nombre EXACTO del repo   // OJO: nombre EXACTO del repo
const DELIM = ";";

// candidatos para detectar columnas
const CLIENT_CANDIDATES = ["CLIENTE", "CLIENTE / OBRA", "CLIENTE NRO.", "OBRA", "ALMACEN", "ALMACÉN"];
// filtro "CLASIFICACIÓN" en Demoras (toma la columna CARACTER DE GC)
const GC_CANDIDATES = ["CARACTER DE GC", "CARÁCTER DE GC", "CARACTER GC", "CARACTER_DE_GC", "CARACTER DE GRUPO DE COMPRAS"];
...
function filteredRows() {
  // base por CLIENTE + CLASIFICACIÓN (CARACTER DE GC)
  let rows = data;
...
function renderGC(rowsBase) {
  // Filtro "CLASIFICACIÓN" (CARACTER DE GC)
  ...
  // conservar selección previa (si existe)
  const prevSet = new Set([...sel.selectedOptions].map(o => o.value));
  ...
  const base = (rowsBase && rowsBase.length) ? rowsBase : data;
