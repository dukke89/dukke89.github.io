...
function animateValue(id, start, end, duration, isPct = false) {
    ...
    if (!obj) return;
    if (start === end) {
        obj.textContent = isPct ? fmtPct01(end) : fmtInt(end);
        return;
    }
    ...
    function step(timestamp) {
        ...
        obj.textContent = isPct ? fmtPct01(current) : fmtInt(Math.floor(current));
        if (progress < 1) {
...
function renderGC() {
    const container = document.getElementById("clasifList");
    if (!container) return;

    if (!GC_COL) return;

    // Capture previous checked state before re-render
    ...
    const allWasChecked = (() => {
        ...
        return !allCb || allCb.checked; // default all checked on first render
    })();
    ...
    for (const g of gcs) {
        ...
        cb.type = "checkbox";
        cb.value = g;
        // On first load (prevChecked empty and allWasChecked) → all checked
    ...
    // "Todos" logic: when Todos is toggled, set all items
    ...
    // Individual item: uncheck Todos if any item is unchecked
    ...
    const hint = document.getElementById("clasifHint");
    if (hint) hint.textContent = `Columna: ${GC_COL}`;
}

/**
 * Returns array of selected classification values (normed).
...
function getCheckedClasif() {
    ...
    if (!container) return [];
    const allCb = container.querySelector(".check-all-cb");
    if (allCb && allCb.checked) return []; // all checked → no filter
...
function buildMesSelect(rows) {
    ...
    // default: Todos, para que el mes no cambie al aplicar otro filtro
    const hasPrevValid = [...prevSet].some(v => v && v !== "__ALL__" && months.includes(v));
    if (!hasPrevValid) {
        // Select "Todos" (no specific month)
...
function countDemoras(rows) {
    ...
    // Ya vienen filtrados globalmente por filteredRows()
    return rows.length;
}
...
function updateKPIs() {
    ...
    // Animación simple: del valor actual (o 0) al nuevo
    const prevDem = parseInt(document.getElementById("kpiDemorasMes").dataset.v || "0");
    document.getElementById("kpiDemorasMes").dataset.v = dem;
    animateValue("kpiDemorasMes", prevDem, dem, 800);
    ...
    const pct = t.total ? (t.bestVal / t.total) : 0;
    ...
    const prevPct = parseFloat(document.getElementById("kpiTopPct").dataset.v || "0");
    document.getElementById("kpiTopPct").dataset.v = pct;
    animateValue("kpiTopPct", prevPct, pct, 800, true);
}
...
function buildChartMes() {
    ...
    // Excluir áreas que no tengan datos en el set filtrado (opcional) 
    // o simplemente mostrar todas las AREA_COLS detectadas.
    const visibleAreas = AREA_COLS.filter(area => {
        const na = norm(area);
        if (na === norm("EQUIPOS MENORES")) return false; // Mantener exclusión visual si es puramente técnica
        return true;
    });
    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: (p) => {
                ...
                return `
                    <div style="padding: 10px 14px; border-left: 5px solid ${col};">
                        <div style="font-size: 11px; color: #64748b; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">${m}</div>
                        <div style="font-size: 14px; color: #1e293b; font-weight: 800; margin-bottom: 4px;">${p.seriesName}</div>
                        <div style="display: flex; align-items: baseline; gap: 8px;">
                            <span style="font-size: 18px; font-weight: 900; color: #0f172a;">${p.value}</span>
                            <span style="font-size: 13px; font-weight: 700; color: ${col}; background: ${col}15; padding: 2px 6px; border-radius: 4px;">${pct}</span>
                        </div>
                    </div>
                `;
            }
...
function buildChartAreas() {
    ...
    const option = {
        tooltip: {
            ...
            formatter: (p) => {
                ...
                return `
                    <div style="padding: 10px 14px; border-left: 5px solid ${col};">
                        <div style="font-size: 11px; color: #64748b; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">Áreas de demora</div>
                        <div style="font-size: 14px; color: #1e293b; font-weight: 800; margin-bottom: 4px;">${getDisplayName(p.name)}</div>
                        <div style="display: flex; align-items: baseline; gap: 8px;">
                            <span style="font-size: 18px; font-weight: 900; color: #0f172a;">${fmtInt(p.value)}</span>
                            <span style="font-size: 13px; font-weight: 700; color: ${col}; background: ${col}15; padding: 2px 6px; border-radius: 4px;">${pct.toFixed(1).replace(".", ",")}%</span>
                        </div>
                    </div>
                `;
            }
    ...
    // ✅ Sincronizar porcentajes de la leyenda al ocultar/mostrar ítems
    chartAreas.on('legendselectchanged', (params) => {
        const selected = params.selected;
        let vTotal = 0;
        items.forEach(it => {
            if (selected[it.name] !== false) vTotal += it.value;
        });

        chartAreas.setOption({
            legend: {
                formatter: (name) => {
                    const it = items.find(x => x.name === name);
                    const v = it ? it.value : 0;
    ...
    chartMotivos.setOption({
        tooltip: {
            ...
            formatter: (p) => {
                ...
                return `
                    <div style="padding: 10px 14px; border-left: 5px solid ${col};">
                        <div style="font-size: 11px; color: #64748b; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">Motivo de demora</div>
                        <div style="font-size: 14px; color: #1e293b; font-weight: 800; margin-bottom: 4px;">${getDisplayName(p.name)}</div>
                        <div style="display: flex; align-items: baseline; gap: 8px;">
                            <span style="font-size: 18px; font-weight: 900; color: #0f172a;">${fmtInt(p.value)}</span>
                            <span style="font-size: 13px; font-weight: 700; color: ${col}; background: ${col}15; padding: 2px 6px; border-radius: 4px;">${pct.toFixed(1).replace(".", ",")}%</span>
...
function buildTabla() {
    ...
    // header - Excluir áreas técnicas si aplica
    const visibleAreas = AREA_COLS.filter(area => {
        const na = norm(area);
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(csvUrl)
        ...
        .then(text => {
            ...
            // clasifSelect replaced by checkboxes – event listeners wired in renderGC()
