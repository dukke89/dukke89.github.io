...
function buildChartMes() {
  // ✅ ACTUALIZADO: Barras agrupadas por ÁREA con Scroll y Etiquetas Verticales
  ...
  // Pre-calcular totales por mes para porcentajes
  const montlyTotals = new Map();
  months.forEach(m => {
    const rowsM = rows.filter(r => getMonthKeyFromRow(r) === m);
    // Total de demoras de ese mes (count de filas que tienen alguna área marcada)
    // OJO: La lógica anterior usaba 'countDemoras' que es rows.length.
    // Aquí asumimos que suma de valores de áreas = total demoras (si cada fila tiene 1 área).
    // Si una fila puede tener varias áreas, el % será sobre la suma de incidencias.
    let sum = 0;
    const mp = aggAreas(rowsM); // Map<Area, Count>
    for (const v of mp.values()) sum += v;
    montlyTotals.set(m, sum);
  });

  // Configurar series (una por Área)
  const seriesBars = AREA_COLS.map((areaName, idx) => ({
    name: areaName,
    type: "bar",
    barGap: '10%',
    barCategoryGap: '20%',
    ...
    label: {
      ...
      formatter: (params) => {
        ...
        // Formato: "394 - 47,0% - CADENA DE SUMINISTRO"
        return `${v} - ${pct} - ${areaName}`;
      },
      fontWeight: 700,
      fontSize: 10,
      color: '#000' // Negro o contraste alto
    }
  }));

  // Eliminar series vacías (opcional, pero limpia el gráfico)
  // const activeSeries = seriesBars.filter(s => s.data.some(d => d.value > 0));
  ...
  const option = {
    ...
    grid: {
      ...
      right: 20,
      top: 30,
      bottom: 80, // espacio para el zoom slider
      containLabel: true
    },
    dataZoom: [
      {
        ...
        xAxisIndex: 0,
        start: 0,
        end: 100, // Mostrar todo inicialmente, usuario puede hacer zoom
        bottom: 35,
        height: 20
      },
      {
    ...
    xAxis: {
      ...
      axisLabel: {
        fontWeight: 700,
        interval: 0 // Mostrar todos los meses
      },
