...
function buildMesSelect(rows) {
  ...
  // Scroll horizontal "normal": asignar ancho al contenedor según cantidad de meses
  const chartMesEl = document.getElementById("chartMes");
  if (chartMesEl) {
    // ~4 meses visibles => cada mes ocupa ~220px
    const perMonthPx = 220;
    const minPx = 1200;
    const w = Math.max(minPx, months.length * perMonthPx);
    chartMesEl.style.width = w + "px";
  }
  const prevSet = new Set([...sel.selectedOptions].map(o => o.value));
...
function buildChartMes() {
  ...
  // Scroll horizontal "normal": ancho según cantidad de meses (para ver ~4 meses)
  const chartMesEl = document.getElementById("chartMes");
  if (chartMesEl) {
    const perMonthPx = 320;   // más ancho => fuerza scroll
    const minPx = 1400;
    const w = Math.max(minPx, months.length * perMonthPx);
    chartMesEl.style.width = w + "px";
  }
