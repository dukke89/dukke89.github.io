...
function buildChartAreas() {
  ...
  // aplicar estilo por item
  dataPie = dataPie.map((d, i) => ({ ...d, itemStyle: { color: palette[i % palette.length] } }));
const chart = echarts.getInstanceByDom(el) || echarts.init(el, null, { renderer: "canvas" });
  ...
  const option = {
    ...
    legend: {
      ...
      formatter: (name) => {
        const item = dataPie.find(d => d.name === name);
...
${pct.toFixed(1).replace(".", ",")}%`;
        ...
        labelLine: { length: 26, length2: 18, smooth: true },
        labelLayout: (params) => {
          // empuja etiquetas hacia afuera para evitar cortes
          const isLeft = params.labelRect.x < (params.rect.x + params.rect.width / 2);
          const points = params.labelLinePoints;
          points[2][0] = isLeft ? params.labelRect.x : (params.labelRect.x + params.labelRect.width);
          return { labelLinePoints: points, hideOverlap: false };
        },
        data: dataPie,
        // no “apaga” el resto al hover
