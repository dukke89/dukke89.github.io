    ...
    series: [
      {
        ...
        data: (() => {
          // Paleta SEGURA (Sin rojos, sin naranjas oscuros, sin rosas)
          // Usamos tonos fríos o neutros. El ROJO (#dc2626) será EXCLUSIVO para el MAX.
          const safePalette = [
            "#2563eb", // Azul royal
            "#16a34a", // Verde
            "#d97706", // Ocre (Amarillo oscuro)
            "#7c3aed", // Violeta fuerte
            "#0891b2", // Cian oscuro
            "#4b5563", // Gris oscuro
            "#4f46e5", // Índigo
            "#65a30d", // Lima
          ...
          return items.map((it) => {
            const isMax = it.value === maxVal;
            // Indice estable basado en el nombre
            const idx = stableNames.indexOf(it.name);
            // Asignar color de la paleta segura
            let finalColor = safePalette[idx % safePalette.length];

            // Lógica ESTRICTA: Solo el MAX es ROJO.
            // Ningún otro elemento puede ser rojo.
            if (isMax) {
              finalColor = "#dc2626"; // Rojo Intenso Alerta
            }
