...
function buildChartMes() {
  ...
  // total por mes (para % en etiquetas)
  const totalsForPct = months.map(m => {
    const rowsM = rows.filter(r => getMonthKeyFromRow(r) === m);
    const mp = aggAreas(rowsM);
    let s = 0;
    AREA_COLS.forEach(a => { s += (Number(mp.get(a)) || 0); });
    return s;
  });
  const seriesBars = areaNames.map(a => ({
    ...
    label: {
      ...
      formatter: (p) => {
        ...
        if (!v) return "";
        const total = (totalsForPct[p.dataIndex] || 0);
  ...
  const option = {
    ...
    grid: { left: 50, right: 30, top: 70, bottom: 105, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: {
    ...
    dataZoom: [
      {
        ...
        show: true,
        xAxisIndex: 0,
        startValue: months[Math.max(0, months.length - 4)],
        endValue: months[months.length - 1],
        height: 26,
        bottom: 10,
        showDetail: false,
        brushSelect: false,
        ...
        fillerColor: "rgba(15,23,42,0.25)"
      },
