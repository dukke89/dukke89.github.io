...
function buildChartMes() {
  ...
  const option = {
    ...
    grid: { left: 40, right: 20, top: 60, bottom: 70, containLabel: true },
    // Mostrar solo 4 meses y el resto con barra de desplazamiento visible
    dataZoom: [
      {
        type: "slider",
        show: true,
    ...
    series: seriesBars.map(s => ({
      ...
      label: {
        ...
        formatter: (p) => {
          ...
          if (!v) return "";
          const total = Number(totals[p.dataIndex]) || 0;
          const pct = total ? (v / total) * 100 : 0;
          const pctTxt = pct.toFixed(1).replace(".", ",") + "%";
          return `${fmtInt(v)} - ${pctTxt} - ${p.seriesName}`;
        },
        fontWeight: 800
      }
    }))
