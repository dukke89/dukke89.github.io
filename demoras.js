...
// Alias por compatibilidad con formateos usados en otros bloques.
// En este reporte, el porcentaje se maneja como 0..1.
const fmtPct = fmtPct01;
...
function buildMesSelect(rows) {
  ...
  // ancho dinámico para scroll horizontal "normal" (sin dataZoom):
  // aprox 3 meses visibles -> cada mes ocupa ~320px
  const perMonthPx = 320;
  const totalWidth = Math.max(960, months.length * perMonthPx);
  const domMes = document.getElementById("chartMes");
  if (domMes) {
    domMes.style.setProperty('width', totalWidth + 'px', 'important');
  }
...
function buildChartMes() {
  // ✅ Migrado a Apache ECharts: barras por ÁREA (mes en eje X) + línea de tendencia (total)
  const el = document.getElementById("chartMes");
  if (!el || typeof echarts === "undefined") return;

  const rows = filteredRows(); // respeta CLIENTE + CLASIFICACIÓN (no depende del MES)
  const months = [...new Set(rows.map(getMonthKeyFromRow).filter(Boolean))]
    .sort((a,b) => monthSortKey(a) - monthSortKey(b));

  // si no hay meses / datos
  ...
  // armar matriz: por cada mes, contar por área (misma lógica que la tabla de Áreas)
  let monthTotals = [];
  const areaNames = AREA_COLS.slice(); // estable
  const seriesBars = areaNames.map(a => ({
    ...
    label: {
      ...
      formatter: (p) => {
        ...
        if (!v) return "";
        const totalMes = Number(monthTotals?.[p.dataIndex] || 0);
        const pct01 = totalMes ? (v / totalMes) : 0;
        return `${fmtInt(v)} - ${fmtPct(pct01)} - ${p.seriesName}`;
      }
    }
    }));

  // total por mes para la línea (sum de áreas; si no cuadra, igual sirve como tendencia visual)
  const totals = months.map((m, idx) => {
    const sumAreas = seriesBars.reduce((acc, s) => acc + (Number(s.data[idx]) || 0), 0);
    return sumAreas;
  });
...
// (re)crear instancia
  ...
  const option = {
    ...
    legend: { bottom: 160, type: "scroll" },
    grid: { left: 40, right: 20, top: 20, bottom: 60, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: {
        rotate: 35, // estilo "bar-label-rotation"
        fontWeight: 800
      }
    },
    yAxis: { type: "value" },
  ...
  if (!chartMesResizeBound) {
    ...
    window.addEventListener(
      "resize",
      () => {
        if (chartMes && typeof chartMes.resize === "function") chartMes.resize();
      },
      { passive: true }
    );
  }
...
function buildChartMotivos() {
  ...
  const dataPie = sums.map((s,i)=>({
    ...s,
    itemStyle: i===maxIdx
      ? { color: "#dc3545", borderWidth: 4, shadowBlur: 14, shadowColor: "rgba(0,0,0,0.30)" }
...
fetch(csvUrl)
    ...
    .then(text => {
      ...
      document.getElementById("mesSelect")?.addEventListener("change", (e) => { enforceAllOption(e.target);
        updateKPIs();
        buildChartAreas();
        buildChartMotivos();
      });
...
// Ajustar tamaño del gráfico mensual al redimensionar ventana
window.addEventListener("resize", () => {
  try {
    if (typeof chartMes !== "undefined" && chartMes) chartMes.resize();
  } catch(e){}
});
