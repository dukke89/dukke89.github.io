...
// Filtro "CLASIFICACIÓN" (en Demoras viene como "CARACTER DE GC")
const GC_CANDIDATES = ["CARACTER DE GC", "CARÁCTER DE GC", "CARACTER GC", "CARACTER_DE_GC", "CARACTER DE GRUPO DE COMPRAS", "CARACTER"];
...
function downloadFilteredCsv() {
  ...
  const gc = selLabel("gcSelect").replace(/[^\w\-]+/g, "_");
  ...
  a.download = `DEMORAS_filtrado_${cliente}_${gc}_${mes}.csv`;
...
function filteredRows() {
  ...
  const gcs = getSelValues("gcSelect");
...
function renderGC(rowsBase) {
  const sel = document.getElementById("gcSelect");
  if (!sel) return;
  ...
  const vals = [...new Set((rowsBase || data).map(r => clean(r[GC_COL])).filter(Boolean))]
    .sort((a,b) => a.localeCompare(b, "es"));
...
fetch(csvUrl)
    ...
    .then(text => {
      ...
      if (document.getElementById("gcHint")) {
        document.getElementById("gcHint").textContent = GC_COL ? `Columna: ${GC_COL}` : "Columna: (no encontrada)";
      }
      ...
      document.getElementById("gcSelect")?.addEventListener("change", (e) => { enforceAllOption(e.target); applyAll(); });
