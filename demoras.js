...
function buildChartMes() {
  ...
  const seriesBars = areaNames.map(a => ({
    ...
    label: {
      ...
      formatter: (p) => {
        ...
        if (!v) return "";
        const total = (Array.isArray(totals) ? (Number(totals[p.dataIndex]) || 0) : 0);
        const pct = total ? (v / total) : 0;
        return `${fmtInt(v)} - ${fmtPct01(pct)} - ${String(p.seriesName || "").toUpperCase()}`;
      },
      fontWeight: 700,
  ...
  const option = {
    ...
    legend: { top: 6, left: "center", type: "scroll" },
    grid: { left: 50, right: 20, top: 60, bottom: 85, containLabel: true },
    dataZoom: [
      {
        type: "slider",
        xAxisIndex: 0,
        height: 18,
        bottom: 40,
        startValue: Math.max(0, months.length - 4),
        endValue: Math.max(0, months.length - 1),
        showDetail: false,
        brushSelect: false,
        showDataShadow: true,
        borderColor: "rgba(0,0,0,0.35)",
        fillerColor: "rgba(0,0,0,0.18)",
