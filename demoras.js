...
function buildChartAreas() {
  ...
  // detectar “STOCK NULO” para pintarlo rojo (si existe en tus labels)
  const isStockNulo = (name) => {
    const n = norm(name);
    return n.includes("STOCK NULO") || n === "STOCK NULO";
  };
  ...
  dataPie = dataPie.map((d, i) => {
    if (isStockNulo(d.name)) {
      return {
        ...d,
        itemStyle: { color: "#dc2626" },  // rojo stock nulo
