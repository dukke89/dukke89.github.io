...
function buildChartMes() {
  ...
  const seriesBars = AREA_COLS.map((areaName) => ({
    name: areaName,
    type: "bar",
    barGap: '10%',
    barCategoryGap: '30%', // un poco más de aire entre grupos
    data: months.map(m => {
      const rowsM = rows.filter(r => getMonthKeyFromRow(r) === m);
      const mp = aggAreas(rowsM);
      ...
      return {
        value: val,
        month: m,
        isMax: isMax, // flag para usar en formatter/rich
        area: areaName
      };
    }),
    label: {
      ...
      formatter: (params) => {
        ...
        // Si es el máximo, devolvemos formato con Rich Text para resaltar
        if (d.isMax) {
      ...
      rich: {
        max: {
          color: '#fff',
          backgroundColor: '#ef4444', // Fondo Rojo Intenso para el TOP
          padding: [4, 6],
          borderRadius: 4,
          fontWeight: 800,
          fontSize: 11,
    ...
    dataZoom: [
      {
        ...
        show: true,
        xAxisIndex: 0,
        startValue: 0, // Mostrar desde el primer mes
        endValue: 4,   // Mostrar aprox 5 meses inicialmente (scroll fijo)
        bottom: 40,
        height: 22,
        zoomLock: true, // Bloquear zoom para que solo sea Scroll (PAN)
        brushSelect: false
      },
      {
        type: 'inside',
    ...
    series: [
      {
        ...
        data: (() => {
          // Paleta distintiva (Azules, Verdes, Naranjas, Violetas, pero SIN ROSA ni ROJO CLARO)
          const distinctColors = [
            "#2563eb", // blue-600 (Azul fuerte)
            "#16a34a", // green-600 (Verde)
            "#d97706", // amber-600 (Ocre)
            "#9333ea", // purple-600 (Violeta)
            "#0891b2", // cyan-600 (Cian oscuro)
            "#6b7280", // gray-500 (Gris)
            "#4f46e5", // indigo-600
            "#65a30d", // lime-600
            "#ca8a04", // yellow-600 (Amarillo oscuro)
            "#0d9488"  // teal-600
          ];

          // Ordenar por valor descendente para asignar colores consistentemente si se desea,
          // o usar el orden alfabético stableNames.
          // Aquí usamos stableNames junto con la paleta.
          return items.map((it) => {
            const isMax = it.value === maxVal;
            // Buscar índice en stableNames para asignar color
            const idx = stableNames.indexOf(it.name);
            const col = distinctColors[idx % distinctColors.length];

            // Si es max, resaltamos? El usuario pidió evitar repetir rojos.
            // MEJOR ESTRATEGIA: Si es el MAX, usar rojo intenso (#dc2626).
            // Para los otros, usar la paleta pero EVITANDO el rojo si es posible o usar un rojo distinto.
            // La paleta de arriba tiene un 'rose' (#f43f5e) que es distinto al #dc2626.
            ...
            if (isMax) {
              finalColor = "#dc2626"; // Rojo Fuerte para el Top 1
              finalBorder = 3;
              shadowBlur = 10;
