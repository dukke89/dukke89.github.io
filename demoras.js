...
// clasificación (igual que Cumplimiento)
const CLASIF2_CANDIDATES = ["CLASIFICACION", "CLASIFICACIÓN", "CLASIFICACION 2", "CLASIFICACIÓN 2", "CLASIFICACION2", "CLASIFICACION_2"];
...
function detectColumns() {
  ...
  CLASIF2_COL = findCol(CLASIF2_CANDIDATES);
...
function filteredRows() {
  ...
  if (c2s.length && CLASIF2_COL) {
    const set2 = new Set(c2s);
    rows = rows.filter(r => set2.has(clean(r[CLASIF2_COL])));
  }
...
function renderClasif2(rowsBase){
  ...
  if (!CLASIF2_COL){
    if (hint) hint.textContent = "Columna clasificación: (no encontrada)";
    sel.disabled = true;
  ...
  if (hint) hint.textContent = `Columna clasificación: ${CLASIF2_COL}`;
  sel.disabled = false;
  ...
  const vals = uniqSorted((rowsBase || data).map(r => r[CLASIF2_COL]));
  for (const v of vals){
    const o = document.createElement("option");
    o.value = v;
    o.textContent = v;
  ...
  if (!hasPrev) optAll.selected = true;
  enforceAllOption(sel);
}
...
============================ */
function applyAll() {
  // base: solo CLIENTE (para poblar CLASIFICACIÓN sin auto-filtrarse)
  let rowsBase = data;
  const cs = getSelValues("clienteSelect");
  if (cs.length && CLIENT_COL){
    const set = new Set(cs);
    rowsBase = data.filter(r => set.has(clean(r[CLIENT_COL])));
  }

  renderClasif2(rowsBase);

  const rows = filteredRows(); // CLIENTE + CLASIFICACIÓN
  buildMesSelect(rows);
