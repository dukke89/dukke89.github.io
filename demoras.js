...
/* ============================
   CSV FETCH (fallback + cachebust)
   - Evita problemas por MAY/MIN y cache en GitHub Pages
============================ */
...
async function fetchCsvText() {
  ...
  for (const base of CSV_CANDIDATES) {
    const url = `${base}?v=${Date.now()}`; // cache-bust
    try {
      ...
      // si devuelve html (404 de GH pages) lo detectamos
      if (/<html/i.test(t) && /<body/i.test(t)) {
        errors.push(`${base} -> devolvió HTML (probable 404)`);
        continue;
      }
      return { text: t, used: base };
    } catch (e) {
      errors.push(`${base} -> ${e.message || e}`);
    }
  }
  throw new Error("No pude cargar el CSV. Probé: " + errors.join(" | "));
}
...
(async () => {
    try {
      ...
      // opcional: mostrar en consola qué archivo se usó
      console.log("[DEMORAS] CSV usado:", used);
      ...
      showError(
        "Error cargando CSV. " +
        "Revisá que el archivo exista en el repo (misma carpeta que demoras.html) " +
        "y que el nombre coincida (mayúsculas/minúsculas). " +
        "
