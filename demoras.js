...
function buildChartMes() {
  ...
  const seriesBars = areaNames.map(a => ({
    ...
    label: {
      ...
      formatter: (p) => {
        ...
        if (!v) return "";
        const monthTotal = Number(totals?.[p.dataIndex] || 0);
  ...
  const _n = months.length;
  const _zoomStart = _n > _visibleMonths ? (100 - (_visibleMonths / _n) * 100) : 0;
const option = {
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" } },
    legend: { bottom: 0, type: "scroll" },
    grid: { left: 40, right: 20, top: 20, bottom: 150, containLabel: true },
    xAxis: {
      type: "category",
      data: months,
      axisLabel: {
