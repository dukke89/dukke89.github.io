...
function aggAreas(rows) {
    ...
    // Encontrar la columna ALMACEN para redirigir EQUIPOS MENORES
    const almacenCol = AREA_COLS.find(a => norm(a) === norm("ALMACEN") || norm(a) === norm("ALMACÉN"));

    for (const r of rows) {
        // Si la fila es EQUIPOS MENORES, sumar su conteo a ALMACEN
        const esEquipoMenor = GC_COL && norm(clean(r[GC_COL])) === norm("EQUIPOS MENORES");
        if (esEquipoMenor && almacenCol) {
            out.set(almacenCol, (out.get(almacenCol) || 0) + 1);
            continue;
        }
...
function buildChartMes() {
    ...
    // Excluir la columna EQUIPOS MENORES de las series (sus datos ya están redirigidos a ALMACEN)
