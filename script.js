...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      const filtered = rows.filter(r => r[idx.estado].trim() === 'CUMPLIDO');
      ...
      if (filtered.length === 0) {
        ...
        msg.textContent = '⚠️ No se encontró ningún pedido con estado CUMPLIDO.';
        msg.style.textAlign = 'center';
        msg.style.color = '#ff5c5c';
        document.querySelector('.table-section').appendChild(msg);
      }
    })
    .catch(err => {
      console.error(err);
