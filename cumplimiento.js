...
function applyAll() {
  updateViewSummary();
  try{ syncTopBadges(document.getElementById('viewSummary')?.textContent || ''); }catch(e){}
...
/* ============================
   VIEW SUMMARY & KPI ALERTS
============================ */
function updateViewSummary(){
  const c = document.getElementById("clienteSelect")?.value || "Todos";
  const cl2 = document.getElementById("clasif2Select")?.value || "Todos";
  const gc = document.getElementById("gcocSelect")?.value || "Todos";
  const m = document.getElementById("mesSelect")?.value || "-";
  const el = document.getElementById("viewSummary");
  if (el) el.textContent = `Vista actual: ${c} · ${cl2} · ${gc} · Mes: ${m}`;
}

function applyKpiAlerts(){
  const at = document.getElementById("kpiATpct");
  const ne = document.getElementById("kpiNOpct");
  if (at){
    const v = parseFloat(at.textContent.replace(",", "."));
    at.closest(".kpi")?.classList.toggle("alert-red", isFinite(v) && v < 75);
  }
  if (ne){
    const v = parseFloat(ne.textContent.replace(",", "."));
    ne.closest(".kpi")?.classList.toggle("alert-red", isFinite(v) && v > 10);
  }
}

/* ============================
   TOP BADGES SYNC (solo UI)
============================ */
function syncTopBadges(viewText){
  const t1 = document.getElementById("viewSummaryTop");
  if (t1 && viewText) t1.textContent = viewText.replace(/^Vista actual:\s*/,'');
  const last = document.getElementById("lastUpdate");
  const t2 = document.getElementById("lastUpdateTop");
  if (last && t2) t2.textContent = "Datos: " + (last.textContent || "--/--/----");
}
