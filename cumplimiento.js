  ...
  const option = {
    ...
    series: [

      {
        ...
        markLine: [
          // MarkLine para el 75% (gris punteado)
          {
            silent: true,
            symbol: ["none", "none"],
            label: {
            ...
            data: (() => {
              ...
              // Encontrar el índice donde cambia a 78%
              let changeIndex = -1;
              for (let i = 0; i < months.length; i++) {
                if (getGoalForMonth(months[i]) === 78) {
                  changeIndex = i;
                  break;
                }
              }

              // Si no hay cambio, línea completa
              if (changeIndex === -1) {
                return [{ yAxis: 75, value: 75 }];
              }

              // Si hay cambio, línea solo hasta antes del cambio
              if (changeIndex > 0) {
                return [[
                  { coord: [-0.5, 75], value: 75 },
                  { coord: [changeIndex - 1 + 0.5, 75] }
                ]];
              }
          ...
          // MarkLine para el 78% (índigo sólido)
          {
            silent: true,
            symbol: ["none", "none"],
            label: {
            ...
            data: (() => {
              ...
              console.log("=== 78% MARKLINE DEBUG ===");

              // Encontrar el índice donde cambia a 78%
              let changeIndex = -1;
              for (let i = 0; i < months.length; i++) {
                const goal = getGoalForMonth(months[i]);
                if (goal === 78 && changeIndex === -1) {
                  changeIndex = i;
                  break;
                }
              }

              console.log("78% starts at index:", changeIndex);

              // Si no hay meses con 78%, no mostrar nada
              if (changeIndex === -1) {
                return [];
              }

              // Línea del 78% desde el inicio del mes hasta el final
              const segment = [
                { coord: [changeIndex - 0.5, 78], value: 78 },
                { coord: [changeIndex + 0.5, 78] }
              ];

              console.log("78% segment:", JSON.stringify(segment));
              return [segment];
            })(),
            z: 100,
            zlevel: 10
