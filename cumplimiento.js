...
/* ============================
   MES TITLE (panel "MES SELECCIONADO")
   - 1 mes: "CUMPLIMIENTO - MES DE <MES> <AÑO>"
   - varios: "CUMPLIMIENTO - MESES SELECCIONADOS"
   - todos/sin selección: "CUMPLIMIENTO - TODOS LOS MESES"
============================ */
const MONTH_NAMES = {
  "01": "ENERO",
  "02": "FEBRERO",
...
function updateMesTitleFromSelect(){
  ...
  if (!titleEl || !sel) return;

  const vals = [...sel.selectedOptions]
    .map(o => o.value)
  ...
  const v = vals[0]; // yyyy-mm
  const parts = v.split("-");
  if (parts.length === 2){
    const year = parts[0];
    const month = parts[1];
    const mesTxt = MONTH_NAMES[month] || month;
    titleEl.textContent = `CUMPLIMIENTO - MES DE ${mesTxt} ${year}`;
  } else {
    titleEl.textContent = `CUMPLIMIENTO - MES ${v}`;
  }
}
}
...
function buildMesSelect(rows) {
  ...
  // actualizar título del panel de "MES SELECCIONADO"
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      // listeners
      document.getElementById("clienteSelect")?.addEventListener("change", (e) => { enforceAllOption(e.target);
      ...
      document.getElementById("clasif2Select")?.addEventListener("change", (e) => { enforceAllOption(e.target);
