...
function buildChartMes(rows) {
  ...
  // Encontrar indice de cambio a 2026 (78%)
  const idx2026 = months.findIndex(m => getGoalForMonth(m) === 78);

  // Datos Objetivo 75%
  const goal75Data = months.map((m, i) => {
    if (getGoalForMonth(m) !== 75) return null;
    // Si es el ultimo mes de 75% (el siguiente es 78 o se termina), poner label a la izquierda
    const isLast75 = (i === months.length - 1) || (getGoalForMonth(months[i + 1]) === 78);
    if (isLast75) {
      return {
        value: 75,
        label: {
          show: true,
          position: "left", // Poner a la izquierda como pidió el usuario
          formatter: "Obj 75%",
  ...
  // Datos Objetivo 78%
  const goal78Data = months.map(m => getGoalForMonth(m) === 78 ? 78 : null);
  ...
  const option = {
    ...
    series: [

      {
        ...
        type: "bar",
        stack: "pct",
        clip: false, // Permitir etiquetas fuera del grid
      ...
      // === NUEVAS SERIES DE OBJETIVO ===
      {
        ...
        connectNulls: false,
        lineStyle: { type: "dashed", width: 2, color: "#374151" },
        // Label se maneja en data para posicionarlo a la izquierda en el último punto
        zlevel: 20, z: 20
      },
      {
        name: "Objetivo 78%",
        ...
        symbol: "none",
        connectNulls: false,
        lineStyle: { type: "solid", width: 3, color: "#4f46e5" },
        zlevel: 21, z: 21,
        // Usamos endLabel habitual para el 78% (a la derecha)
        endLabel: {
          show: true,
          formatter: "Obj 78%",
          fontWeight: 800,
        ...
        // IMPORTANTISIMO: MarkLine para dibujar la linea cuando hay un solo punto (ej. Enero 2026)
        markLine: {
          silent: true,
          symbol: ["none", "none"],
          data: [
            [
              { coord: [idx2026 - 0.5, 78], lineStyle: { type: "solid", width: 3, color: "#4f46e5" } },
              { coord: [months.length - 1 + 0.5, 78] }
            ]
          ],
          z: 21, zlevel: 21
        }
