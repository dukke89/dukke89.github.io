    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: (p) => {
            ...
            // 🔴 MENOR AL 75%: destacar
            if (pct < 75) return `⚠ ${fmtInt(q)}
(${pctRound}%)`;
...
(${pctRound}%)`;
          ...
          color: (p) => (+p.data < 75 ? "#7f1d1d" : "#ffffff"),
          backgroundColor: (p) => (+p.data < 75 ? "rgba(239,68,68,0.20)" : "transparent"),
          borderColor: (p) => (+p.data < 75 ? "#ef4444" : "transparent"),
          borderWidth: (p) => (+p.data < 75 ? 1 : 0),
          borderRadius: 4,
          padding: [2, 4]
        },
