...
function drawSparkline(containerId, values, colorHex) {
  ...
  const points = values.map((val, i) => {
    ...
    const y = h - ((val - min) / range) * h * 0.8;
    return `${x.toFixed(1)},${y.toFixed(1)}`;
  });
  const lineD = `M ${points[0]} ` + points.slice(1).map(p => `L ${p}`).join(' ');
  const pathD = `${lineD} L ${w},${h} L 0,${h} Z`;
  el.innerHTML = `
<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" style="width:100%; height:100%; display:block;">
  ...
  <path d="${pathD}" fill="url(#grad-${containerId})" stroke="none"/>
  <path d="${lineD}" fill="none" stroke="${colorHex}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>`;
}

function updateKPIsMonthly(rows, months) {
  const trendTotal = [], trendAT = [], trendFT = [], trendNO = [], trendDemora = [];
  for (const m of months) {
    const cur = calcMonthTotals(rows, m);
    trendTotal.push(cur.total);
    trendAT.push(isFinite(cur.pctAT) ? cur.pctAT : 0);
    trendFT.push(isFinite(cur.pctFT) ? cur.pctFT : 0);
    trendNO.push(isFinite(cur.pctNO) ? cur.pctNO : 0);
    const mRows = rows.filter(r => getMonthKeyFromRow(r) === m);
    const avgD = avgDelay(mRows);
    trendDemora.push(isNaN(avgD) ? 0 : avgD);
  }
  drawSparkline("sparkTotalMes", trendTotal, "#3b82f6");
  drawSparkline("sparkATmes", trendAT, "#10b981");
  drawSparkline("sparkFTmes", trendFT, "#f59e0b");
  drawSparkline("sparkNOmes", trendNO, "#ef4444");
  drawSparkline("sparkDemoraMes", trendDemora, "#64748b");
