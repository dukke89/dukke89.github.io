...
function buildChartMes(rows) {
  ...
  // Datos para series de objetivo (Lineas)
  // 75%: Solo si el goal es 75.
  const goal75Data = months.map(m => getGoalForMonth(m) === 75 ? 75 : null);
  // 78%: Solo si el goal es 78.
    ...
    series: [
      ...
      // === NUEVAS SERIES DE OBJETIVO ===
      {
        ...
        endLabel: {
          ...
          padding: [4, 6],
          borderRadius: 4,
          offset: [10, 0] // desplazar un poco a la derecha
        }
      },
      {
        name: "Objetivo 78%",
