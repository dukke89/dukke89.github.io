...

      {
        ...
        markLine: {
          ...
          data: (() => {
            ...
            // Uso de xAxis/yAxis explícitos para soportar índices fraccionarios sin problemas
            ...
            for (let i = 1; i < months.length; i++) {
              ...
              if (val !== currentVal) {
                ...
                segments.push([
                  { xAxis: startIdx - 0.49, yAxis: currentVal, value: currentVal },
                  { xAxis: i - 1 + 0.49, yAxis: currentVal }
                ]);
            ...
            // Último segmento: Etiqueta forzada 'end' (final de linea)
            // 'right' a veces falla en markLine, usamos 'end' standard
            segments.push([
              { coord: [startIdx - 0.49, currentVal], value: currentVal },
              {
                coord: [months.length - 1 + 0.49, currentVal],
                label: {
                  show: true,
                  position: "end",
                  // align: "left", // 'end' ya alinea a la derecha del punto
                  // Sin offset manual para alinear con 'Lím 7 d' que usa defaults
                  formatter: "Obj {c}%"
                },
                lineStyle: {
                  color: "#4f46e5", // Indigo vibrante para diferenciar
                  type: "solid",    // Sólida para destacar mas
                  width: 3
                }
              }
            ]);
          ...
          // Force high Z to be on top of bars
          z: 100,
          zlevel: 10
        },
      },
