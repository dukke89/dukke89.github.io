...
function buildChartTendencia(rows) {
  ...
  // Promedio mensual acumulado de AT % (promedio de pAT desde el primer mes hasta el mes actual)
  const pAT_acum = [];
  let accAT = 0;
  for (let i = 0; i < pAT.length; i++) {
    accAT += (isFinite(pAT[i]) ? pAT[i] : 0);
...
{
  name: "A Tiempo % (Prom. mensual acumulado)",
  type: "line",
