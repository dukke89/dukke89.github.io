...
function updateKPIsMonthly(rows, months) {
  ...
  const elATmes = document.getElementById("kpiATmes");
  const currentGoal = getGoalForMonth(mes);
  if (elATmes) elATmes.style.color = (isFinite(cur.pctAT) && cur.pctAT >= (currentGoal / 100)) ? "#16a34a" : "#ef4444";
...
function buildChartMes(rows) {
  ...
  // Índices calculados de forma compatible
  const idx2025Last = months.reduce((acc, m, i) => m.startsWith("2025") ? i : acc, -1);
  const idx2026First = months.findIndex(m => m.startsWith("2026"));
    ...
    markData.push([
      {
        ...
        label: { show: true, formatter: "Obj 75%", position: "end", backgroundColor: '#374151', color: '#fff', padding: [4, 6], borderRadius: 4 }
      },
      {
        yAxis: 75,
        xAxis: idx2025Last + 0.5
      }
    ]);
  }
  if (idx2026First !== -1) {
    markData.push([
      {
        name: "2026",
        yAxis: 78,
        xAxis: idx2026First - 0.5,
        lineStyle: { type: "dashed", width: 2, color: "#374151" },
        label: { show: true, formatter: "Obj 78%", position: "end", backgroundColor: '#374151', color: '#fff', padding: [4, 6], borderRadius: 4 }
      },
      {
        yAxis: 78,
    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: (p) => {
            ...
            // Si es menor al objetivo dinámico
            if (pct < currentGoal) {
    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: (p) => {
            const v = +p.data || 0;
            const currentGoal = getGoalForMonth(months[p.dataIndex]);
