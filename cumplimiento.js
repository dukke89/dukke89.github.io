...
function buildChartMes(rows){
  ...
  for(const r of rows){
    ...
    if(!mk) continue;
    const at = toNumber(r[AT_COL]);
    const ft = toNumber(r[FT_COL]);
    const no = toNumber(r[NO_COL]);
    const dem = toNumber(r[DEMORA_COL]);
    const cur = byMonth.get(mk) || {at:0, ft:0, no:0, demSum:0, demN:0};
  ...
  const atQ = months.map(m=>byMonth.get(m).at);
  const ftQ = months.map(m=>byMonth.get(m).ft);
  const noQ = months.map(m=>byMonth.get(m).no);
  const tot = months.map((m,i)=> atQ[i]+ftQ[i]+noQ[i]);
  const atP = months.map((m,i)=> tot[i]? (atQ[i]/tot[i])*100 : 0);
  const ftP = months.map((m,i)=> tot[i]? (ftQ[i]/tot[i])*100 : 0);
  const noP = months.map((m,i)=> tot[i]? (noQ[i]/tot[i])*100 : 0);
  const demAvg = months.map(m=>{
    const o=byMonth.get(m);
    return o.demN? (o.demSum/o.demN) : null;
  });

  // labels pretty: "Jan 2025" style ya viene monthKey -> "Jan 2025"
  const labels = months;

  if(__chartMes) __chartMes.destroy();

  __chartMes = new Chart(ctx, {
    type: "bar",
    data: {
      labels,
      datasets: [
        ...
        { label: "Promedio días de demora", type:"line", data: demAvg, yAxisID:"y2",
          borderColor: COLORS.demora, backgroundColor: COLORS.demora,
          pointRadius: 3, pointHoverRadius: 4, borderWidth: 2, tension: 0.25, order:1
        }
      ]
    },
    options: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx)=>{
              ...
              if(ds.type==="line") return ` ${ds.label}: ${ctx.parsed.y?.toFixed(1)} d`;
              const i = ctx.dataIndex;
              const q = ds.label.includes("AT") ? atQ[i] : ds.label.includes("FT") ? ftQ[i] : noQ[i];
              return ` ${ds.label}: ${fmtInt(q)} (${ctx.parsed.y.toFixed(0)}%)`;
            }
          }
        },
        datalabels: {
          color: (ctx)=> ctx.dataset.type==="line" ? "#111827" : "#ffffff",
          ...
          formatter: (v, ctx)=>{
            ...
            if(v < 6) return ""; // evita ruido en segmentos chicos
            const q = ds.label.includes("AT") ? atQ[i] : ds.label.includes("FT") ? ftQ[i] : noQ[i];
            return `${fmtInt(q)} (${Math.round(v)}%)`;
          },
          font: (ctx)=> ({ size: ctx.dataset.type==="line" ? 11 : 10, weight: ctx.dataset.type==="line" ? "700":"600" })
        }
      },
      scales: {
        ...
        y2: {
          ...
          title: { display: true, text: "Días de demora" }
        }
      }
    },
...
function buildChartTendencia(rows){
  const ctx = _getCtx("chartTendencia");
  if(!ctx) return;
  ...
  for(const r of rows){
    ...
    const ft = toNumber(r[FT_COL]);
    const no = toNumber(r[NO_COL]);
    const cur = byMonth.get(mk) || {at:0, ft:0, no:0};
    cur.at += isNaN(at)?0:at;
    cur.ft += isNaN(ft)?0:ft;
    cur.no += isNaN(no)?0:no;
    byMonth.set(mk, cur);
  ...
  if(__chartTend) __chartTend.destroy();

  __chartTend = new Chart(ctx, {
    type: "line",
    data: {
      labels: months,
      datasets: [
        { label: "A Tiempo %", data: atP, borderColor: COLORS.at, backgroundColor: COLORS.at,
          pointRadius: 2.5, borderWidth: 2, tension: 0.25 },
        { label: "Fuera Tiempo %", data: ftP, borderColor: COLORS.ft, backgroundColor: COLORS.ft,
          pointRadius: 2.5, borderWidth: 2, tension: 0.25 },
        { label: "No Entregados %", data: noP, borderColor: COLORS.no, backgroundColor: COLORS.no,
          pointRadius: 2.5, borderWidth: 2, tension: 0.25 }
      ]
    },
    options: {
