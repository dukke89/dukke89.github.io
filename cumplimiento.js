...
async function fetchWithCache(url, storeKey) {
  ...
  const db = await new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = e => {
      ...
      if (!db.objectStoreNames.contains(storeName)) db.createObjectStore(storeName, { keyPath: 'url' });
    };
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetchWithCache(csvUrl + "?t=" + window.CACHE_BUSTER, "csv_data_cache")
    .then(text => {
