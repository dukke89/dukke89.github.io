...
function getGoalForMonth(monthStr) {
  // monthStr format: "YYYY-MM"
  if (!monthStr) return 75;
  const y = parseInt(monthStr.split("-")[0], 10);
  return (y >= 2026) ? 78 : 75;
}
...
function rowsByClienteBase() {
  ...
  if (CLASIF2_COL) {
    rows = rows.filter(r => {
      const clasif = clean(r[CLASIF2_COL]).toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      const target = "EQUIPOS MENORES".normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      return clasif !== target;
    });
  }
...
function renderClasif2(rowsBase) {
  ...
  // Excluir EQUIPOS MENORES de las opciones
  const vals = uniqSorted(rowsBase.map(r => r[CLASIF2_COL]))
    .filter(v => {
      const norm = clean(v).toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      return norm !== "EQUIPOS MENORES";
    });
...
function updateKPIsGeneral(rows) {
  ...
  // Nota: Para acumulado general usamos 75% como base o podríamos hacerlo ponderado.
  // Ante la duda, si hay mezclados, mantenemos 75% o usamos lógica estricta. 
  // Dejamos 75% como base general si no hay mes específico.
...
function updateKPIsMonthly(rows, months) {
  ...
  if (!ms.length) {
    ...
    if (elATmes) elATmes.style.color = (isFinite(pctAT) && pctAT >= 0.75) ? "#16a34a" : "#ef4444";
  ...
  const goal = getGoalForMonth(mes);
  if (elATmes) elATmes.style.color = (isFinite(cur.pctAT) && cur.pctAT >= (goal / 100)) ? "#16a34a" : "#ef4444";
  ...
  const option = {
    ...
    series: [

      {
        ...
        type: "bar",
        stack: "pct",
        // ✅ Dinámico según año (75% o 78%)
        data: pAT.map((v, i) => {
          const val = +(+v).toFixed(4);
          const currentGoal = getGoalForMonth(months[i]);
          if (val < currentGoal) {
            return {
              value: val,
              itemStyle: {
        ...
        label: {
          ...
          formatter: (p) => {
            ...
            // Si es menor al goal, style warn
            if (pct < currentGoal) {
        ...
        markLine: {
          ...
          data: (() => {
            // Generar segmentos de línea por grupos de meta
            // Ejemplo: Ene-Dic 2025 (75%), Ene 2026 (78%)
            if (!months.length) return [];

            const segments = [];
            let startIdx = 0;
            let currentVal = getGoalForMonth(months[0]);

            for (let i = 1; i < months.length; i++) {
              const val = getGoalForMonth(months[i]);
              if (val !== currentVal) {
                // Cierro segmento anterior
                // coord: [indice, valor]
                // Para que se vea contínuo visualmente hasta el cambio, podemos usar coord 'xAxis' index.
                // ECharts markLine con coord dibuja de punto A a punto B.
                // Queremos una línea horizontal desde startIdx hasta i-1.
                // Pero para que cubra todo el ancho de la categoría, lo ideal es startIdx - 0.5 hasta endIdx + 0.5?
                // MarkLine en category axis usa indices enteros. 
                // Si pongo coord: [startIdx, val], coord: [i-1, val] dibuja de centro a centro.
                // Para cubrir extremos, podemos usar x 'min' o 'max' si es todo, pero aqui es por tramos.
                // Simple: de startIdx a i-1. 
                segments.push([
                  { coord: [startIdx, currentVal], value: currentVal },
                  { coord: [i - 1, currentVal] }
                ]);
            ...
            segments.push([
              { coord: [startIdx, currentVal], value: currentVal },
              { coord: [months.length - 1, currentVal] }
            ]);

            // Si hay un solo segmento que cubre todo, mejor usar yAxis para que vaya de borde a borde (cross layout)
            if (segments.length === 1 && segments[0][0].coord[0] === 0 && segments[0][1].coord[0] === months.length - 1) {
              return [{ yAxis: currentVal, value: currentVal }];
            }
