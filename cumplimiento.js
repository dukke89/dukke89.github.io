...
function buildMesSelect(rows) {
  ...
  // Opción "Todos" (ver todos los meses)
  const opt0 = document.createElement("option");
  opt0.value = "";
  ...
  sel.value = (prevSelected === "" || months.includes(prevSelected))
    ? prevSelected
    : (months[months.length - 1] || "");
const hint = document.getElementById("mesHint");
  if (hint) hint.textContent = sel.value ? `Mes seleccionado: ${sel.value}` : "Mes seleccionado: Todos";
...
(${pctRound}%)`;
  ...
  chartMes.setOption(__applyEchartsPolish(option), true);
      ...
      },
{
  ...
  chartTendencia.setOption(__applyEchartsPolish(option), true);
