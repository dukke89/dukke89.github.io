...
function buildChartTendencia(rows) {
  ...
  // Promedio móvil (2 meses) de AT %
  // - Primer mes: igual al %AT
  // - Luego: (mes anterior + mes actual) / 2
  const pAT_MA2 = pAT.map((v, i) => {
    if (i === 0) return v;
    return (pAT[i - 1] + v) / 2;
  });
  ...
  const option = {
    ...
    series: [
      ...
      {
        name: "Promedio AT acumulado",
        type: "line",
        data: pAT_MA2.map(v => +(+v).toFixed(2)),
        symbolSize: 6,
        connectNulls: true,
        lineStyle: { width: 3, type: "dashed", color: "#7c3aed" },
