...
function rowsByClienteBase() {
  ...
  // RECLASIFICACIÓN: EQUIPOS MENORES -> ALMACEN
  // En lugar de excluirlos, los tratamos como ALMACEN en todos los cálculos
  if (CLASIF2_COL) {
    const targetNorm = "EQUIPOS MENORES".normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    rows = rows.map(r => {
      const clasif = clean(r[CLASIF2_COL]).toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      if (clasif === targetNorm) {
        // Devolvemos una copia de la fila con la clasificación cambiada a ALMACEN
        return { ...r, [CLASIF2_COL]: "ALMACEN" };
      }
      return r;
    });
  }
