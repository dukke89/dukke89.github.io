...
============================ */
function buildChartMes_plotly(rows) {
  const agg = new Map();
  ...
  const tAT = {
    ...
    y: pAT,
    customdata: qAT,
    text: qAT.map((q,i)=> q ? `${fmtInt(q)} (${pAT[i].toFixed(0)}%)` : ""),
    textposition: "inside",
    hovertemplate: "%{x}<br>AT: %{customdata} (%{y:.1f}%)<extra></extra>",
  };
  ...
  const tFT = {
    ...
    y: pFT,
    customdata: qFT,
    text: qFT.map((q,i)=> q ? `${fmtInt(q)} (${pFT[i].toFixed(0)}%)` : ""),
    textposition: "inside",
    hovertemplate: "%{x}<br>FT: %{customdata} (%{y:.1f}%)<extra></extra>",
  };
  ...
  const tNO = {
    ...
    y: pNO,
    customdata: qNO,
    text: qNO.map((q,i)=> q ? `${fmtInt(q)} (${pNO[i].toFixed(0)}%)` : ""),
    textposition: "inside",
    hovertemplate: "%{x}<br>No entregados: %{customdata} (%{y:.1f}%)<extra></extra>",
  };
  const tDem = {
    type: "scatter",
    mode: "lines+markers",
    ...
    y: avgDem,
    yaxis: "y2",
    hovertemplate: "%{x}<br>Promedio demora: %{y:.1f} días<extra></extra>",
  };


  const layout = {
    annotations: makeDemoraAnnotations(months, avgDem),

    barmode: "stack",
    margin: { l: 55, r: 70, t: 10, b: 45 },
    xaxis: { tickangle: 0, automargin: true },
    yaxis: { title: "%", range: [0, 100], ticksuffix: "%", gridcolor: "rgba(0,0,0,0.08)" },
    yaxis2: { title: "Días de demora", overlaying: "y", side: "right", showgrid: false, rangemode: "tozero" },
    legend: { orientation: "h", y: -0.2 },
  };

  const config = { displayModeBar: false, displaylogo: false, responsive: true };

  Plotly.react(el, [tAT, tFT, tNO, tDem], layout, config);
}

/* ============================
   CHART 2: Trend lines
============================ */
function buildChartTendencia_plotly(rows) {
  const agg = new Map();
  const monthsSet = new Set();
...
// ====== Chart.js render (con fallback a Plotly) ======
let __chartMesJS = null;
let __chartTendJS = null;

function buildChartMes(rows){
  try{
    if(typeof Chart === "undefined") return buildChartMes_plotly(rows);
    const ctx = _ensureCanvas("chartMes","chartMesCanvas");
    if(!ctx) return buildChartMes_plotly(rows);

    // agregación por mes (reusa helpers existentes)
    const agg = new Map();
    for(const r of rows){
      const mk = (typeof monthKeyFromRow === "function") ? monthKeyFromRow(r) : null;
      if(!mk) continue;
      const at = toNumber(r[COL_AT]);
      const ft = toNumber(r[COL_FT]);
      const no = toNumber(r[COL_NO]);
      const dem = toNumber(r[COL_DEMORA_PROM]);
      const cur = agg.get(mk) || {at:0, ft:0, no:0, demSum:0, demN:0};
      cur.at += isNaN(at)?0:at;
      cur.ft += isNaN(ft)?0:ft;
      cur.no += isNaN(no)?0:no;
      if(!isNaN(dem)){ cur.demSum += dem; cur.demN += 1; }
      agg.set(mk, cur);
    }

    const labels = [...agg.keys()].sort();
    const atQ = labels.map(m=>agg.get(m).at);
    const ftQ = labels.map(m=>agg.get(m).ft);
    const noQ = labels.map(m=>agg.get(m).no);
    const tot = labels.map((m,i)=>atQ[i]+ftQ[i]+noQ[i]);
    const atP = labels.map((m,i)=> tot[i]? (atQ[i]/tot[i])*100 : 0);
    const ftP = labels.map((m,i)=> tot[i]? (ftQ[i]/tot[i])*100 : 0);
    const noP = labels.map((m,i)=> tot[i]? (noQ[i]/tot[i])*100 : 0);
    const demAvg = labels.map(m=>{
      const o=agg.get(m);
      return o.demN? (o.demSum/o.demN) : null;
    });

    if(__chartMesJS) __chartMesJS.destroy();
    const plugins = (typeof ChartDataLabels !== "undefined") ? [ChartDataLabels] : [];

    __chartMesJS = new Chart(ctx, {
      type:"bar",
      data:{
        labels,
        datasets:[
          {label:"Entregados AT", data:atP, backgroundColor:COLORS.at, stack:"pct", order:2},
          {label:"Entregados FT", data:ftP, backgroundColor:COLORS.ft, stack:"pct", order:2},
          {label:"No entregados", data:noP, backgroundColor:COLORS.no, stack:"pct", order:2},
          {label:"Promedio días de demora", type:"line", data:demAvg, yAxisID:"y2",
           borderColor:COLORS.demora, backgroundColor:COLORS.demora, pointRadius:3, borderWidth:2, tension:0.25, order:1}
        ]
      },
      options:{
        responsive:true,
        ...
        plugins:{
          ...
          tooltip:{callbacks:{
            label:(c)=>{
              if(c.dataset.type==="line") return ` ${c.dataset.label}: ${c.parsed.y?.toFixed(1)} d`;
              const i=c.dataIndex;
              const q = c.dataset.label.includes("AT")?atQ[i]:c.dataset.label.includes("FT")?ftQ[i]:noQ[i];
              return ` ${c.dataset.label}: ${fmtInt(q)} (${c.parsed.y.toFixed(0)}%)`;
            }
          }},
          datalabels:{
            display:true,
            color:(c)=> c.dataset.type==="line" ? "#111827" : "#ffffff",
            ...
            font:(c)=>({size: c.dataset.type==="line"?11:10, weight:"600"})
          }
        },
        scales:{
          x:{stacked:true, ticks:{maxRotation:0}},
          y:{stacked:true, min:0, max:100, ticks:{callback:(v)=>`${v}%`}, title:{display:true,text:"%"}},
          y2:{position:"right", grid:{drawOnChartArea:false}, title:{display:true,text:"Días de demora"}}
        }
      },
      plugins
    });
  }catch(e){
    // si algo falla, volvemos a Plotly para no romper la página
    return buildChartMes_plotly(rows);
  }
}

function buildChartTendencia(rows){
  try{
    if(typeof Chart === "undefined") return buildChartTendencia_plotly(rows);
    const ctx = _ensureCanvas("chartTendencia","chartTendCanvas");
    if(!ctx) return buildChartTendencia_plotly(rows);
    ...
    for(const r of rows){
      ...
      const ft = toNumber(r[COL_FT]);
      const no = toNumber(r[COL_NO]);
      const cur = agg.get(mk) || {at:0, ft:0, no:0};
      cur.at += isNaN(at)?0:at;
      cur.ft += isNaN(ft)?0:ft;
      cur.no += isNaN(no)?0:no;
      agg.set(mk, cur);
    ...
    if(__chartTendJS) __chartTendJS.destroy();
    const plugins = (typeof ChartDataLabels !== "undefined") ? [ChartDataLabels] : [];

    __chartTendJS = new Chart(ctx,{
      type:"line",
      data:{labels, datasets:[
        {label:"A Tiempo %", data:atP, borderColor:COLORS.at, backgroundColor:COLORS.at, pointRadius:2.5, borderWidth:2, tension:0.25},
        {label:"Fuera Tiempo %", data:ftP, borderColor:COLORS.ft, backgroundColor:COLORS.ft, pointRadius:2.5, borderWidth:2, tension:0.25},
        {label:"No Entregados %", data:noP, borderColor:COLORS.no, backgroundColor:COLORS.no, pointRadius:2.5, borderWidth:2, tension:0.25},
      ]},
      options:{
        responsive:true,
        maintainAspectRatio:false,
        ...
        scales:{ y:{min:0,max:100,ticks:{callback:(v)=>`${v}%`},title:{display:true,text:"%"}}}
      },
      plugins
    });
  }catch(e){
    return buildChartTendencia_plotly(rows);
  }
}
